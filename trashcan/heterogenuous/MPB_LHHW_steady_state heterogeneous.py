"""
Moving Packed Bed (MPB) Reactor Model — Steady-State, Heterogeneous Energy Balance
====================================================================================
...
Heterogeneous energy balance: separate T_g(z) and T_s(z), coupled by gas-solid
heat exchange h_gs·a_p·(T_s − T_g).  Each phase has its own stable IVP:

    Gas   (z = 0 → L):
        u_g·ρ_g_mol·Cp_mix · dT_g/dz =
            (−ΔH_r)·ρ_bed_cat·r          ← reaction heat → gas
          + h_gs·a_p·(T_s − T_g)          ← solid-gas exchange
          − U_a·(T_g − T_wall)            ← wall cooling

    Solid (ζ = 0 → L, ζ = L−z):
        u_s·ρ_bed_tot·Cp_cat · dT_s/dζ =
            (−ΔH_ads)·ρ_bed_ads·ads       ← adsorption heat → solid
          + h_gs·a_p·(T_g − T_s)          ← gas-solid exchange

Both denominators are always positive → both IVPs are always stable, regardless
of u_s.  This correctly captures solid convective heat transport without the BVP
instability of the pseudo-homogeneous counter-current formulation.

Solved by decoupled Gauss-Seidel iteration:
  1. Gas IVP   (z = 0 → L, BDF): fixed q(z), T_s(z) → gives C(z), T_g(z)
  2. Solid IVP (ζ = 0 → L, BDF): fixed C_H2O(z), T_g(z) → gives q(z), T_s(z)
  3. Repeat until max(Δq/scale_q, ΔT_s/T_K) < tol
...
Cp_cat = 1100.0        # [J/(kg·K)]      — catalyst/sorbent heat capacity, literature estimate from bareschino and mancusi
Cp_CO2 = 45.4;  Cp_H2 = 29.3;  Cp_CH4 = 46.9;  Cp_H2O = 34.2;  Cp_N2 = 29.5  # [J/(mol·K)] — NIST at ~550 K
U_a    = 1000.0        # [W/(m³_bed·K)]  — volumetric wall heat transfer coeff., own estimate, h = 100 W/(M2*K)

# --- Heterogeneous heat transfer parameters ---
a_p  = 6*(1-eps_b)/d_p  # [m²/m³_bed]  — specific particle surface area, geometric
h_gs = 250.0             # [W/(m²·K)]   — gas-solid heat transfer coefficient, Ranz-Marshall estimate
...
print(f"     a_p={a_p:.0f} m2/m3, h_gs={h_gs:.0f} W/(m2.K), "
      f"h_gs*a_p={h_gs*a_p/1e6:.2f} MW/(m3.K)")
# endregion


# region 2. INHERITED FUNCTIONS, validated by Wei's experiments
# =============================================================================
def P_sat_bar(T_K):
    """
...
def reaction_rate_SI(T_K, p_CO2, p_H2, p_CH4, p_H2O):
    ...
    f_eq         : approach-to-equilibrium factor = max(1 - Q/K_eq, 0),
                   where Q = p_CH4*p_H2O^2/(K_eq*p_CO2*p_H2^4).
                   Shuts the rate off smoothly as the system nears equilibrium.
    DEN^2        : bimolecular LHHW denominator — OH* (from H2O) and H2 compete
                   for active sites; squared because two sites are involved.
    *1000        : converts mol -> mmol to match k_ref units [mmol/(kg_cat·s·bar^0.5)].
    ...
    k     = k_ref*np.exp(-Ea_k/R_gas*(1.0/T_K - 1.0/T_ref_K))     # Arrhenius [mmol/(kg·s·bar^0.5)]
    ...
    f_eq  = np.maximum(1.0 - np.where(np.isfinite(beta), beta, 1e10), 0.0)  # approach-to-eq.
    DEN   = (1.0 + K_OH*np.maximum(p_H2O,0)/p_H2_s**0.5           # OH* site blocking
             + K_H2*p_H2_s**0.5 + K_mix*p_CO2_s**0.5)                  # H2 and CO2 adsorption
...
def equilibrium_conversion(T_K_val):
    ...
    Mole fractions from inlet (y_CO2=0.04, y_H2=0.16, y_CH4=0.80, P=1 bar):
        d = 1 - 0.08*X  (net mole change: -2 mol per mol CO2 reacted)
        p_CH4 = (0.80+0.04X)/d,  p_H2O = 0.08X/d,
        p_CO2 = 0.04(1-X)/d,     p_H2  = 0.16(1-X)/d
    """
...
def solve_mpb(u_s, u_g, T_K, C_in_CO2, C_in_H2, C_in_CH4,
              max_iter=200, tol=1e-4, N=100, q_init=None, T_wall=None):
    """
    Counter-current MPB at steady state, heterogeneous energy balance.

    Decoupled Gauss-Seidel iteration:
      - Gas IVP  : z = 0 -> L (BDF), uses q(z) and T_s(z) from previous iteration
                   T_g(z) is the 5th state variable
      - Solid IVP: zeta = L-z, zeta=0->L (BDF), uses C_H2O(z) and T_g(z) from gas step
                   T_s(z) is the 2nd state variable alongside q(z)

    q_init: optional initial q(z) profile [mol/kg] on N points — warm-start.
    T_wall: wall / furnace temperature [K]; defaults to T_K.
    """
    if T_wall is None:
        T_wall = T_K
    ...
    if q_init is not None:
        ...
        q_prof = np.zeros(N)
    T_s_prof = T_K * np.ones(N)   # solid temperature profile, updated each iteration
    ...
    for it in range(max_iter):
        ...
        T_s_fn = interp1d(z_grid, T_s_prof, kind='linear',
                          bounds_error=False, fill_value=(T_s_prof[0], T_s_prof[-1]))

        # ── Step 1: gas IVP from z=0 to z=L ─────────────────────────────────
        # Reaction heat goes to gas; solid-gas exchange couples T_g and T_s.
        # Gas-only denominator (u_g*rho_g_mol*Cp_mix) is always positive → stable IVP.
        def gas_rhs(z, y):
            C_CO2_l = max(y[0], 0.0)
            C_H2_l  = max(y[1], 0.0)
            C_CH4_l = max(y[2], 0.0)
            C_H2O_l = max(y[3], 0.0)
            T_g_l   = max(y[4], 200.0)
            T_s_l   = max(float(T_s_fn(z)), 200.0)
            q_l     = max(float(q_fn(z)), 0.0)

            p_CO2 = C_CO2_l*R_gas*T_g_l/1e5
            p_H2  = C_H2_l *R_gas*T_g_l/1e5
            p_CH4 = C_CH4_l*R_gas*T_g_l/1e5
            p_H2O = C_H2O_l*R_gas*T_g_l/1e5

            r   = float(reaction_rate_SI(T_g_l,
                        np.array([p_CO2]), np.array([p_H2]),
                        np.array([p_CH4]), np.array([p_H2O]))[0])
            # adsorption uses solid temperature for isotherm and LDF
            qs  = float(q_star(T_s_l, np.array([p_H2O]))[0])
            Kl  = float(K_LDF(T_s_l,  np.array([p_H2O]))[0])
            ads = Kl*(qs - q_l)

            rho_g_mol_l = P_Pa / (R_gas * T_g_l)
            y_CO2l = p_CO2/P_bar; y_H2l = p_H2/P_bar
            y_CH4l = p_CH4/P_bar; y_H2Ol = p_H2O/P_bar
            Cp_mix_l = (y_CO2l*Cp_CO2 + y_H2l*Cp_H2 + y_CH4l*Cp_CH4
                        + y_H2Ol*Cp_H2O
                        + max(1.0 - y_CO2l - y_H2l - y_CH4l - y_H2Ol, 0.0)*Cp_CH4)
            denom    = u_g * rho_g_mol_l * Cp_mix_l   # always > 0
            Q_rxn    = (-dH_r) * rho_bed_cat * r
            Q_exch   = h_gs * a_p * (T_s_l - T_g_l)
            Q_wall_  = U_a * (T_g_l - T_wall)
            dTg_dz   = (Q_rxn + Q_exch - Q_wall_) / denom
        ...
        gs = solve_ivp(gas_rhs, [0.0, L_b], [C_in_CO2, C_in_H2, C_in_CH4, 0.0, T_K],
                       method='BDF', rtol=1e-4, atol=np.array([1e-8, 1e-8, 1e-8, 1e-8, 1e-2]),
                       t_eval=z_grid, dense_output=False)
        if not gs.success:
            return None

        C_H2O_prof = np.maximum(gs.y[3], 0.0)
        T_g_prof   = np.maximum(gs.y[4], 200.0)
        C_H2O_fn   = interp1d(z_grid, C_H2O_prof, kind='linear',
                               bounds_error=False,
                               fill_value=(C_H2O_prof[0], C_H2O_prof[-1]))
        T_g_fn_new = interp1d(z_grid, T_g_prof, kind='linear',
                               bounds_error=False,
                               fill_value=(T_g_prof[0], T_g_prof[-1]))

        # ── Step 2: solid IVP in zeta = L-z from zeta=0 (z=L) to zeta=L (z=0)
        # State: [q, T_s].  Solid enters at zeta=0 (z=L) with q=0, T_s=T_K.
        # Adsorption heat goes to solid; solid-gas exchange couples T_s and T_g.
        # Solid denominator (u_s*rho_bed_tot*Cp_cat) is always positive → stable IVP.
        def solid_rhs(zeta, y_arr):
            q_val   = max(float(y_arr[0]), 0.0)
            T_s_val = max(float(y_arr[1]), 200.0)
            z_pos   = L_b - float(zeta)
            T_g_loc = float(T_g_fn_new(z_pos))
            p_H2O   = max(float(C_H2O_fn(z_pos)), 0.0)*R_gas*T_g_loc/1e5
            qs  = float(q_star(T_s_val, np.array([p_H2O]))[0])
            Kl  = float(K_LDF(T_s_val,  np.array([p_H2O]))[0])
            ads = Kl*(qs - q_val)
            Q_ads_s  = (-dH_ads) * rho_bed_ads * ads
            Q_exch_s = h_gs * a_p * (T_g_loc - T_s_val)
            dTs_dzeta = (Q_ads_s + Q_exch_s) / (rho_bed_tot * Cp_cat * u_s)
            return [Kl*(qs - q_val)/u_s, dTs_dzeta]

        zeta_eval = np.linspace(0.0, L_b, N)
        ss = solve_ivp(solid_rhs, [0.0, L_b], [0.0, T_K],
                       method='BDF', rtol=1e-4, atol=np.array([1e-8, 1e-2]),
        ...
        Ts_from_zeta = np.maximum(ss.y[1], 200.0)

        sort_idx = np.argsort(z_from_zeta)
        q_new    = np.interp(z_grid, z_from_zeta[sort_idx], q_from_zeta[sort_idx])
        T_s_new  = np.interp(z_grid, z_from_zeta[sort_idx], Ts_from_zeta[sort_idx])

        # Damped update for both q and T_s
        q_prof_new  = 0.5 * q_prof   + 0.5 * q_new
        T_s_prof_new = 0.5 * T_s_prof + 0.5 * T_s_new

        # Convergence: check both q (relative) and T_s (relative to T_K)
        err_q = np.max(np.abs(q_prof_new  - q_prof))   / max(np.max(q_prof_new), 1e-8)
        err_T = np.max(np.abs(T_s_prof_new - T_s_prof)) / T_K
        err   = max(err_q, err_T)

        q_prof   = q_prof_new
        T_s_prof = T_s_prof_new
        if err < tol:
            converged = True
            break
    ...
    # ── Recompute final profiles with converged q and T_s ───────────────────
    q_fn_final   = interp1d(z_grid, q_prof,   kind='linear',
                             bounds_error=False, fill_value=(q_prof[0],   q_prof[-1]))
    T_s_fn_final = interp1d(z_grid, T_s_prof, kind='linear',
                             bounds_error=False, fill_value=(T_s_prof[0], T_s_prof[-1]))
    ...
    def gas_rhs_final(z, y):
        ...
        T_g_l   = max(y[4], 200.0)
        T_s_l   = max(float(T_s_fn_final(z)), 200.0)
        q_l     = max(float(q_fn_final(z)), 0.0)
        p_CO2 = C_CO2_l*R_gas*T_g_l/1e5;  p_H2  = C_H2_l *R_gas*T_g_l/1e5
        p_CH4 = C_CH4_l*R_gas*T_g_l/1e5;  p_H2O = C_H2O_l*R_gas*T_g_l/1e5
        r   = float(reaction_rate_SI(T_g_l, np.array([p_CO2]), np.array([p_H2]),
                                      np.array([p_CH4]), np.array([p_H2O]))[0])
        qs  = float(q_star(T_s_l, np.array([p_H2O]))[0])
        Kl  = float(K_LDF(T_s_l,  np.array([p_H2O]))[0])
        ads = Kl*(qs - q_l)
        ...
        Cp_mix_l = (y_CO2l*Cp_CO2 + y_H2l*Cp_H2 + y_CH4l*Cp_CH4
                    + y_H2Ol*Cp_H2O
                    + max(1.0 - y_CO2l - y_H2l - y_CH4l - y_H2Ol, 0.0)*Cp_CH4)
        denom    = u_g * rho_g_mol_l * Cp_mix_l
        Q_rxn    = (-dH_r) * rho_bed_cat * r
        Q_exch   = h_gs * a_p * (T_s_l - T_g_l)
        Q_wall_  = U_a * (T_g_l - T_wall)
        dTg_dz   = (Q_rxn + Q_exch - Q_wall_) / denom
    ...
    gf = solve_ivp(gas_rhs_final, [0.0, L_b], [C_in_CO2, C_in_H2, C_in_CH4, 0.0, T_K],
    ...
    T_g_fine = np.maximum(gf.y[4], 200.0)
    T_s_fine = np.interp(z_fine, z_grid, T_s_prof)

    p_CO2f = C_CO2f*R_gas*T_g_fine/1e5;  p_H2f  = C_H2f *R_gas*T_g_fine/1e5
    p_CH4f = C_CH4f*R_gas*T_g_fine/1e5;  p_H2Of = C_H2Of*R_gas*T_g_fine/1e5
    r_fine = reaction_rate_SI(T_g_fine, p_CO2f, p_H2f, p_CH4f, p_H2Of)
    X_CO2  = np.clip(1.0 - C_CO2f/C_in_CO2, 0.0, 1.0)

    return dict(z=z_fine, C_CO2=C_CO2f, C_H2=C_H2f, C_CH4=C_CH4f,
                C_H2O=C_H2Of, q=q_fine, T_g=T_g_fine, T_s=T_s_fine,
                T=T_g_fine,   # alias for backward compatibility
...
def _q_physics_init(T_K, u_g, C_in_CO2, C_in_H2, C_in_CH4, N=150):
    ...
    q_prof_f = q_star(T_K, p_H2O_prof)
    p_CO2_f  = np.maximum(sol.y[0], 0.0) * R_gas * T_K / 1e5
    p_H2_f   = np.maximum(sol.y[1], 0.0) * R_gas * T_K / 1e5
    p_CH4_f  = np.maximum(sol.y[2], 0.0) * R_gas * T_K / 1e5
    T_arr    = T_K * np.ones(N)
    r_f      = reaction_rate_SI(T_arr, p_CO2_f, p_H2_f, p_CH4_f, p_H2O_prof)
    X_f      = np.clip(1.0 - np.maximum(sol.y[0], 0.0) / C_in_CO2, 0.0, 1.0)
    profile  = dict(z=z_grid, C_CO2=np.maximum(sol.y[0], 0.0),
                    C_H2O=np.maximum(sol.y[3], 0.0), q=q_prof_f, r=r_f, X_CO2=X_f)
    return dict(q=q_prof_f, X_CO2_noSE=X_CO2_noSE, profile=profile)
...
for T_C in T_IN_LIST:
    ...
    noSE_results[T_C] = _phys
    print(f"  non-SE fixed-bed conversion: {_phys['X_CO2_noSE']*100:.1f}%")
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        if res is not None:
            ...
            print(f"  u_s={u_s*1e3:.4f} mm/s  X={X_out:.1f}%  "
                  f"q(0)={q_out:.3f} mol/kg  [{tag}, {res['n_iter']} iter, err={res['conv_err']:.2e}]"
...
fig.suptitle(f'MPB axial profiles  |  T_in = {T_C_PROF} C  |  heterogeneous, counter-current',
             fontsize=11)
...
if T_C_PROF in noSE_results:
    p0 = noSE_results[T_C_PROF]['profile']
    axes[0,0].plot(p0['z'], p0['C_CO2']*1e3, color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
    axes[0,1].plot(p0['z'], p0['q'],          color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
    axes[1,0].plot(p0['z'], p0['X_CO2']*100,  color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
    axes[1,1].plot(p0['z'], p0['r']*1e3,      color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
...
# ── Plot 4: q profile and C_H2O profile for several u_s ─────────────────────
...
if T_C_PROF in noSE_results:
    p0 = noSE_results[T_C_PROF]['profile']
    ax_q.plot(p0['z'], p0['q'],                          color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
    ax_h.plot(p0['z'], p0['C_H2O']*R_gas*T_C_PROF/1e2, color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
...
# ── Plot 5: T_g and T_s profiles — shows solid convective heat transport ──────
fig, ax_T = plt.subplots(figsize=(9, 5))
fig.suptitle(f'Temperature profiles along bed  |  T_in = {T_C_PROF} C  |  heterogeneous',
             fontsize=11)
T_wall_C = T_C_PROF
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    ...
    r   = e['res']
    lbl = f"u_s = {e['u_s']*1e3:.3f} mm/s"
    ax_T.plot(r['z'], r['T_g'] - 273.15, color=pal[k], lw=2,      label=f"{lbl} T_g")
    ax_T.plot(r['z'], r['T_s'] - 273.15, color=pal[k], lw=1.5, ls='--', label=f"{lbl} T_s")

ax_T.axhline(T_wall_C, color='grey', lw=1.5, ls=':', alpha=0.8, label='T_wall (furnace)')
ax_T.set_xlabel('z [m]', fontsize=10)
ax_T.set_ylabel('T [°C]', fontsize=10)
ax_T.set_title('Gas (solid) and solid (dashed) temperature profiles  —  heterogeneous energy balance',
               fontsize=9)
ax_T.legend(fontsize=6);  ax_T.grid(True, alpha=0.3)
plt.tight_layout();  plt.show()

# ── Plot 6: CO2 mole fraction along bed [mol%] ───────────────────────────────
T_K_prof = T_C_PROF + 273.15
fig, ax6 = plt.subplots(figsize=(9, 5))
fig.suptitle(f'CO2 mole fraction along bed  |  T_in = {T_C_PROF} C  '
             f'(inlet: {y_CO2_in*100:.0f} mol%)', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        continue
    r   = e['res']
    lbl = f"u_s = {e['u_s']*1e3:.3f} mm/s"
    y_CO2 = r['C_CO2'] * R_gas * r['T_g'] / P_Pa * 100   # mol%
    ax6.plot(r['z'], y_CO2, color=pal[k], lw=2, label=lbl)
if T_C_PROF in noSE_results:
    p0 = noSE_results[T_C_PROF]['profile']
    y_CO2_noSE = p0['C_CO2'] * R_gas * T_K_prof / P_Pa * 100
    ax6.plot(p0['z'], y_CO2_noSE, color='grey', lw=2, ls='--', label='u_s = 0 (fixed bed)')
ax6.set_xlabel('z [m]', fontsize=10)
ax6.set_ylabel('y_CO2 [mol%]', fontsize=10)
ax6.legend(fontsize=7);  ax6.grid(True, alpha=0.3)
ax6.axvline(0,   color='tab:blue',   lw=1, ls=':', alpha=0.5)
ax6.axvline(L_b, color='tab:orange', lw=1, ls=':', alpha=0.5)
plt.tight_layout();  plt.show()
# endregion
