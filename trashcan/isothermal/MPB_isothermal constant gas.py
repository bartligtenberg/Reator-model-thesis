...
# Suppress a benign scipy BDF internal warning that fires with stiff equations;
# the solver recovers and the solution is unaffected.
...
# --- MPB scan parameters ---
U_S_LIST = np.array([1, 2, 3, 4, 5, 6, 8, 10]) * 1e-3   # [m/s] — 1, 2, 3, 4, 6, 8 mm/s
...
def solve_mpb(u_s, u_g, T_K, C_in_CO2, C_in_H2, C_in_CH4,
    ...
    for it in range(max_iter):
        ...
        def gas_rhs(z, y):
            ...
            if any(not np.isfinite(v) for v in [r, Kl, ads] + dydz):
                print(f"  [gas_rhs NaN] z={z:.3f}  p_CO2={p_CO2:.2e}  p_H2={p_H2:.2e}  "
                      f"p_H2O={p_H2O:.2e}  r={r:.2e}  Kl={Kl:.2e}  ads={ads:.2e}  "
                      f"dydz={[f'{v:.2e}' for v in dydz]}")
                dydz = [0.0 if not np.isfinite(v) else v for v in dydz]
            return dydz
    ...
    # H2O black-box balance over the reactor [mmol/m2/s]
    #   produced  = 2 * u_g * eps_b * (C_CO2_in - C_CO2(L))   [stoichiometry]
    #   gas_out   = u_g * eps_b * C_H2O(L)                     [gas outlet at z=L]
    #   solid_out = u_s * rho_bed_ads * q(z=0)                 [solid outlet at z=0]
    #   bal = produced - gas_out - solid_out  (should be 0)
    _f        = u_g * eps_b * 1e3
    H2O_prod  = 2.0 * _f * (C_in_CO2 - float(C_CO2f[-1]))
    H2O_gas   = _f * float(C_H2Of[-1])
    H2O_sol   = u_s * rho_bed_ads * float(q_fine[0]) * 1e3
    H2O_bal   = H2O_prod - H2O_gas - H2O_sol
    H2O_rel   = H2O_bal / max(H2O_prod, 1e-30)

    return dict(z=z_fine, C_CO2=C_CO2f, C_H2=C_H2f, C_CH4=C_CH4f,
                C_H2O=C_H2Of, q=q_fine, T=T_fine, r=r_fine, X_CO2=X_CO2,
                converged=converged, n_iter=it+1, err=err,
                H2O_prod=H2O_prod, H2O_gas=H2O_gas,
                H2O_sol=H2O_sol, H2O_bal=H2O_bal, H2O_rel=H2O_rel)
# endregion
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        if res is not None:
            ...
            print(f"  u_s={u_s*1e3:.4f} mm/s  X={X_out:.1f}%  "
                  f"q(0)={q_out:.3f} mol/kg  [{tag}, {res['n_iter']} iter, err={res['err']:.1e}]"
                  f"  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
            print(f"    H2O [mmol/m2/s]: prod={res['H2O_prod']:.4f}  "
                  f"gas_out={res['H2O_gas']:.4f}  solid_out={res['H2O_sol']:.4f}  "
                  f"bal={res['H2O_bal']:.2e}  rel={res['H2O_rel']:.2e}")
