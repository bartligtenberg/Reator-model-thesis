...
rho_ads = 791           # adsorbed-phase density (liquid water) [kg/m³]

# --- Particle density (for K_LDF, per Mette 2014 Eq. 19) ---
# ρ_s in the paper is the adsorbent particle density, derived from the bulk
# density and the interparticle void fraction.
rho_p   = 1400   # [kg/m³]  particle density of sorbent (Bareschino)

# --- Intraparticle transport parameters (Mette 2014, via Bareschino Eq. 24) ---
eps_p   = 0.242             # intraparticle void fraction (pore volume fraction) [-]
tau_p   = 4.0             # tortuosity factor (accounts for winding pore paths) [-]
...
PARAM_SETS = {
    ...
    'Ligtenberg (2026)': {'W0':  190e-6, 'E':  1192.25e3, 'n': 1.55},  # hypothetical set for testing
}
...
def K_LDF_vec(T_K, p_arr, W0, E, n):
    ...
    Formula (Mette 2014, Eq. 19 — pore diffusion control):
        K_LDF = 15 · D_M · ε_p
                ─────────────────────────────────────────────────
                0.5 · d_p² · τ · ρ_p · R · T · (dq*/dP)

    Derivation: the paper writes δ_eff = D_M/μ · ε_s / (ρ_s R_w T ∂X*/∂p)
    with R_w = R/MW_H₂O and X* in [kg/kg]. Since the code uses q* in [mol/kg]
    (= X*/MW_H₂O), the two MW_H₂O factors cancel and ρ_s is the particle density.

    dq*/dP [mol/(kg·Pa)] is the local slope of the isotherm.
...
plt.tight_layout()
import os
out_path = os.path.join(os.path.dirname(__file__), 'bareschino_fig1a.png')
