"""
Moving Packed Bed (MPB) Reactor Model — Steady-State, Adiabatic, Pseudo-Homogeneous
=====================================================================================
...
Adiabatic: no wall heat exchange (U_a = 0).

Pseudo-homogeneous energy balance (T_g = T_s = T):

    (u_g·ρ_g_mol·Cp_mix  −  u_s·ρ_bed·Cp_cat) · dT/dz  =
        (−ΔH_r)·ρ_bed_cat·r  +  (−ΔH_ads)·ρ_bed_ads·ads

The sign of the denominator determines the natural integration direction:

    u_s < u_s*  (gas dominates):   denominator > 0,  T solved in GAS IVP (z = 0 → L)
                                    BC: T(z=0) = T_in
    u_s > u_s*  (solid dominates): denominator < 0,  rewrite in solid direction ζ = L−z:
                                    (u_s·ρ_bed·Cp_cat − u_g·ρ_g·Cp_mix) · dT/dζ = Q_rxn + Q_ads
                                    denominator > 0,  T solved in SOLID IVP (ζ = 0 → L)
                                    BC: T(ζ=0) = T_in  (solid enters at z=L cold)

Both denominators are always positive in their respective directions — no singular
point and no regime-switching hack.
...
print(f"MPB adiabatic corrected: d={d_b*100:.0f} cm, L={L_b:.1f} m, "
...
_K_LDF_MAX = 0.5   # [1/s] — at normal conditions K_LDF ~ 0.01 s^-1; above 0.5 s^-1
                   # adsorption is instantaneous on reactor length scales.
                   # Without this cap, dq*/dp → 0 at high T causes K_LDF → 10^24 s^-1,
                   # making the ODE astronomically stiff and causing solver failure.
...
def solve_mpb(u_s, u_g, T_K, C_in_CO2, C_in_H2, C_in_CH4,
    ...
    """
    Counter-current MPB — adiabatic, pseudo-homogeneous, regime-switching.

    Computes u_s* from inlet conditions.  Branches on u_s vs u_s*:

      u_s < u_s*: gas IVP carries T  (denominator gas_cap − solid_cap > 0)
      u_s > u_s*: solid IVP carries T (denominator solid_cap − gas_cap > 0, in ζ-space)

    Both paths are stable forward IVPs with always-positive denominators.
    """
    solid_cap = u_s * rho_bed_tot * Cp_cat   # [W/(m²·K)] constant

    # Estimate u_s* from inlet conditions
    ...
    gas_dominates = (solid_cap < gas_cap_in)   # True when u_s < u_s*
    ...
    for it in range(max_iter):
        ...
        if gas_dominates:
            ...
            # dT/dz = Q / (gas_cap − solid_cap),  denominator > 0
            def gas_rhs(z, y):
            ...
            # ── SOLID IVP: state = [q, T] ─────────────────────────────────────
            # dT/dζ = Q / (solid_cap − gas_cap_local),  denominator > 0
            # BC: q(ζ=0) = 0,  T(ζ=0) = T_K  (solid enters at z=L, cold)
            #
            # Near u_s*, solid_denom is small → dT/dζ is very large.  Two guards:
            #   1. T_VAL_MAX caps T so that f_eq → 0 and Q_rxn self-extinguishes.
            #   2. max_step=1e-3 prevents the ODE solver jumping over the steep region.
            T_VAL_MAX = T_K + 700.0   # K — above this f_eq ≈ 0, rate self-limits

            def solid_rhs_with_T(zeta, y_arr):
                q_val = max(float(y_arr[0]), 0.0)
                T_val = min(max(float(y_arr[1]), 200.0), T_VAL_MAX)
                z_pos = L_b - float(zeta)
                ...
                # If T is capped, report zero gradient so solver knows it's at the limit
                if float(y_arr[1]) >= T_VAL_MAX:
                    return [Kl*(qs - q_val)/u_s, 0.0]

                return [Kl*(qs - q_val)/u_s,
                        (Q_rxn + Q_ads) / solid_denom]
    ...
    if gas_dominates:
        ...
        C_CO2f = np.maximum(gf.y[0], 0.0);  C_H2f  = np.maximum(gf.y[1], 0.0)
        C_CH4f = np.maximum(gf.y[2], 0.0);  C_H2Of = np.maximum(gf.y[3], 0.0)
        T_fine = np.interp(z_fine, z_grid, T_prof)   # solid IVP gave T(z)
...
def _compute_noSE_adiabatic(T_K, u_g, C_in_CO2, C_in_H2, C_in_CH4, N=300):
    """Fixed-bed (u_s=0) adiabatic reference: gas-only denominator."""
    def rhs(_z, y):
        C_CO2_l = max(y[0], 0.0); C_H2_l  = max(y[1], 0.0)
        C_CH4_l = max(y[2], 0.0); C_H2O_l = max(y[3], 0.0)
        T_l     = max(y[4], 200.0)
...
for T_C in T_IN_LIST:
    ...
    print(f"\n{'='*60}")
    print(f"  T_in = {T_C} C  |  u_g = {u_g*1e3:.1f} mm/s  (ADIABATIC, corrected)")
...
fig.suptitle(f'MPB axial profiles  |  T_in = {T_C_PROF} C  |  adiabatic corrected, counter-current',
             fontsize=11)
...
ax.set_title('MPB adiabatic corrected — CO2 conversion vs solid velocity', fontsize=10)
...
ax.set_title('MPB adiabatic corrected — Sorbent utilisation vs solid velocity', fontsize=10)
...
fig.suptitle(f'H2O profiles  |  T_in = {T_C_PROF} C  |  adiabatic corrected', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        continue
...
fig.suptitle(f'Temperature profile  |  T_in = {T_C_PROF} C  |  adiabatic corrected', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        continue
...
ax_T.set_title('(g) = gas-dominated  |  (s) = solid-dominated — T solved in solid IVP',
               fontsize=9)
...
ax6.set_title('MPB adiabatic corrected — peak temperature rise vs solid velocity', fontsize=10)
