...
# --- Catalyst and sorbent loading ---  updated more realistic masses
f_Ni  = 0.050   # 5.0 wt% Ni
f_Ce  = 0.025   # 2.5 wt% Ce
f_ads = 1.0 - f_Ni - f_Ce   # = 0.925, zeolite 13X fraction

M_total = V_bed * (1 - eps_b) * rho_p
M_cat   = (f_Ni + f_Ce) * M_total   # = 0.075 * M_total
M_ads   = f_ads * M_total            # = 0.925 * M_total
...
# ── Plot 7: Gas velocity profiles ────────────────────────────────────────────
fig, ax7 = plt.subplots(figsize=(9, 5))
fig.suptitle(f'Gas superficial velocity  |  T_in = {T_C_PROF} C  |  '
...
for k, i_us in enumerate(i_plot):
    ...
    r   = e['res']
    lbl = f"u_s = {e['u_s']*1e3:.3f} mm/s"
    ax7.plot(r['z'], r['u_g']*1e3, color=pal[k], lw=2, label=lbl)
if _p0 is not None and 'u_g' in _p0:
    ax7.plot(_p0['z'], _p0['u_g']*1e3, color='k', lw=2, ls='--', label='u_s=0 (fixed bed)')
ax7.axhline(u_g_STP*1e3, color='grey', lw=1.5, ls=':', alpha=0.7, label=f'u_g inlet (STP) = {u_g_STP*1e3:.2f} mm/s')
ax7.set_xlabel('z [m]', fontsize=10)
ax7.set_ylabel('u_g [mm/s]', fontsize=10)
ax7.set_title('Superficial gas velocity along bed  '
              '(decreases: 5→3 mol reaction, H₂O adsorption; increases: T rise)',
              fontsize=9)
ax7.legend(fontsize=7);  ax7.grid(True, alpha=0.3)
ax7.axvline(0,   color='tab:blue',   lw=1, ls=':', alpha=0.5)
ax7.axvline(L_b, color='tab:orange', lw=1, ls=':', alpha=0.5)
plt.tight_layout()
_savefig(f'flux_plot7_gas_velocity_T{T_C_PROF}C.png');  plt.show()
# endregion
