...
Updated parameters vs original energy_balance_check.py:
  - eps_p = 0.242  (zeolite 13X intraparticle porosity, updated)
  - D_M   = 3.36e-9 * T_K**1.75  (Chapman-Enskog molecular diffusivity, updated)
  - Bed masses derived from rho_p, eps_b, V_bed and weight fractions (5% Ni, 2.5% Ce, 92.5% 13X)
...
# =============================================================================
# 1. PARAMETERS  (identical to parent file MPB_flux_form high mass.py)
# =============================================================================
d_b   = 0.050;  L_b  = 2.000
...
d_p   = 2.5e-3;  eps_p = 0.242;  tau_p = 3.0;  rho_p = 1400.0  # updated eps_p

# --- Catalyst and sorbent loading from weight fractions ---
f_Ni  = 0.050   # 5.0 wt% Ni
f_Ce  = 0.025   # 2.5 wt% Ce
f_ads = 1.0 - f_Ni - f_Ce   # = 0.925, zeolite 13X fraction

M_total = V_bed * (1 - eps_b) * rho_p
M_cat   = (f_Ni + f_Ce) * M_total
M_ads   = f_ads * M_total
...
# =============================================================================
# 2. FUNCTIONS  (identical to parent file, updated D_M in K_LDF_vec)
# =============================================================================
def P_sat_bar(T_K):
...
def K_LDF_vec(T_K, p_arr, W0, E, n):
    D_M  = 3.36e-9 * T_K**1.75  # updated: Chapman-Enskog molecular diffusivity [m²/s]
    p    = np.asarray(p_arr, dtype=float)
    dp   = 1.0/1e5
...
print("=" * 65)
print(f"Energy balance check  (updated eps_p, D_M, bed masses)")
print(f"  u_s = {U_S_CHECK*1e3:.2f} mm/s  |  T_in = {T_IN_C} °C  |  U_a = {U_a:.0f} W/(m³·K)")
print(f"  eps_p = {eps_p}  |  M_cat = {M_cat:.4f} kg  |  M_ads = {M_ads:.4f} kg")
print(f"  rho_bed_cat = {rho_bed_cat:.2f}  |  rho_bed_ads = {rho_bed_ads:.2f}  |  rho_bed_tot = {rho_bed_tot:.2f} kg/m³")
...
fig.suptitle(
    ...
    f'eps_p = {eps_p}  |  M_cat = {M_cat:.3f} kg  |  M_ads = {M_ads:.3f} kg',
    fontsize=10)
