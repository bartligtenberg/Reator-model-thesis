...
--------------------------------------------------------------------------------------------
LENGTH-DETERMINATION SWEEP EXTENSION (this file)
--------------------------------------------------------------------------------------------
Instead of sweeping u_s directly, this file sweeps GHSV and DERIVES u_s at each GHSV from a
fixed capacity-margin target LAMBDA_TARGET (the corrected Lambda = u_s*rho_bed_ads*q_star_max /
(2*F_in_CO2) definition established in the lambda-threshold-determination script), using
q_star_max evaluated at the analytic p_H2O bound reached at full CO2 conversion (NOT the
p->P_sat asymptote, which hugely overstates the reachable loading -- same distinction used in
the (u_s, GHSV) heatmap sweep script). Each (GHSV, derived u_s) case is solved with solve_mpb(),
and the z at which X_CO2(z) first crosses X_TARGET (99% by default) is read off by linear
interpolation -- the worst case (largest z_cross) across GHSV_LIST is the required reactor
length before margin.

L_b below is the TRIAL bed length (L_TRIAL) for this sweep, not the real lab-scale L_b_REF=1 m
unit -- long enough that the X_TARGET crossing falls inside the domain for every swept GHSV.
Catalyst/sorbent mass is scaled by L_TRIAL/L_b_REF alongside it, so rho_bed_cat/rho_bed_ads stay
identical to the real 1 m design (same pattern as the heatmap sweep script) -- the trial bed is
a longer bed of the same packing, not a diluted version of the real charge. L_b is this file's
own module-level global, set once below -- it is never restored, but that only affects this
file's own run, not the main/sibling scripts (each is a separate module).
"""
...
LAMBDA_TARGET = 2.5      # [-]  capacity-margin target; u_s is DERIVED from this at each GHSV (not swept independently)
U_S_MIN   = 3e-3          # [m/s]  floor on the derived u_s -- below this, convergence gets unreliable, and practically u_s this low isn't of interest anyway
GHSV_LIST = np.array([0.5, 1.0, 1.5, 2.0, 2.5, 3.0])   # [m3_STP/(kg_ads·h)]  GHSV values to sweep
L_TRIAL   = 2.0           # [m]  trial bed length -- long enough that the X_TARGET crossing falls inside the domain for every swept GHSV (longer than the real 1 m design so the marker isn't cut off)
X_TARGET  = 0.99          # [-]  target CO2 conversion for the length-determining crossing
...
L_b     = L_TRIAL              # [m]   TRIAL bed length for this sweep (see EDITABLE SWEEP PARAMETERS above) -- physically
                                #       longer than the real lab-scale unit so the X_TARGET crossing has a chance to occur
                                #       inside the domain. Catalyst/sorbent mass is scaled up proportionally (see
                                #       bifuctional_mass below) so rho_bed_cat/rho_bed_ads stay IDENTICAL to the real
                                #       L_b_REF=1 m design -- L_b only changes how much bed there is, not its packing density.
...
# --- Inlet molar fluxes [mol/(m²·s)] — GHSV-dependent (see compute_inlet_fluxes()/set_inlet_fluxes()
# in region 2), since GHSV is swept below instead of held fixed. These globals are placeholders,
# overwritten before any solve_mpb call. ---
F_in_CO2 = F_in_H2 = F_in_CH4 = F_total_in = u_g_STP = 0.0

print(f"MPB flux form (length-determination sweep): d={d_b*100:.0f} cm, L_trial={L_b:.1f} m, "
...
print(f"  LAMBDA_TARGET = {LAMBDA_TARGET:.2f}  |  GHSV_LIST = {list(GHSV_LIST)}  |  "
      f"X_TARGET = {X_TARGET:.1%}  |  T = {T_SWEEP_C:.0f} C  |  L_trial = {L_b:.2f} m")
# endregion
...
def compute_inlet_fluxes(GHSV):
    ...
    a fixed module-level GHSV -- needed because the sweep below varies GHSV across GHSV_LIST.
    """
...
# region 4. SOLVE LOOP — GHSV SWEEP (u_s DERIVED FROM LAMBDA_TARGET)
# =============================================================================
def _compute_noSE(T_K, T_wall, N=300):
    ...
    two-IVP iteration. Relies on F_in_* already being set (via set_inlet_fluxes) for the
    GHSV of interest -- used as the conventional-fixed-bed baseline printed alongside each
    GHSV case below.
    """
    def rhs(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
    ...
    return dict(X_CO2_noSE=float(X_f[-1]))                             # single scalar: outlet CO2 conversion, printed alongside each GHSV case below
...
def _q_physics_init(T_K, N=150):
    ...
    return dict(q=q_star(T_K, p_H2O_f),                             # equilibrium loading implied by this (unremoved) p_H2O(z) -- rises with z since water accumulates as the gas reacts along its own direction of travel (z=0->L)
                X_CO2_noSE=float(np.clip(1.0 - F_CO2_f[-1]/F_in_CO2, 0.0, 1.0)))  # outlet conversion for this isothermal pass; unused by callers, just a byproduct
...
def _h2o_balance(u_s, res):
    ...
    Relies on F_in_CO2 (set via set_inlet_fluxes for this case's GHSV) and rho_bed_ads."""
...
def _z_cross(z_arr, X_arr, target):
    """z where X_CO2(z) first crosses `target`; NaN if never reached within L_TRIAL."""
    if z_arr is None or np.max(X_arr) < target:
        return np.nan
    return float(np.interp(target, X_arr, z_arr))

T_SWEEP_K   = T_SWEEP_C + 273.15
GHSV_SORTED = np.sort(GHSV_LIST)   # warm-starting relies on consecutive GHSV values having similar solutions -- solve low-to-high
n_ghsv      = len(GHSV_SORTED)

all_results = {}     # i_ghsv -> {'res', 'u_s', 'GHSV', 'Lambda_check', 'z_cross', 'noSE', 'h2o_balance'}
n_done      = 0
t_run_start = time.perf_counter()

print(f"\n{'='*60}")
print(f"  Length-determination sweep  |  T = {T_SWEEP_C:.0f} C  |  LAMBDA_TARGET = {LAMBDA_TARGET:.2f}  |  L_trial = {L_b:.2f} m")
print(f"{'='*60}")

q_init = None   # warm start carried forward across GHSV_SORTED (ascending)
for i_ghsv, GHSV in enumerate(GHSV_SORTED):
    set_inlet_fluxes(GHSV)

    # q_star_max at T_SWEEP_K, using the analytic p_H2O bound reached at full CO2 conversion
    # (NOT the p->P_sat asymptote -- see file header / lambda-threshold script for why)
    p_H2O_max_bound = P_bar * (2.0*F_in_CO2) / (F_in_CH4 + F_in_CO2 + 2.0*F_in_CO2)   # [bar]
    q_star_max = float(q_star(T_SWEEP_K, np.array([p_H2O_max_bound]))[0])             # [mol/kg]

    u_s = LAMBDA_TARGET * 2.0 * F_in_CO2 / (rho_bed_ads * q_star_max)                 # [m/s]  derived from the fixed capacity margin
    u_s = max(u_s, U_S_MIN)                                                            # floor at U_S_MIN -- below this convergence gets unreliable, and practically not of interest
    Lambda_check = (u_s * rho_bed_ads * q_star_max) / (2.0 * F_in_CO2)                # sanity check -- should equal LAMBDA_TARGET (unless the U_S_MIN floor kicked in)

    if q_init is None:
        q_init = _q_physics_init(T_SWEEP_K)['q'][::-1]    # cold-start guess, built once for the lowest GHSV only
    _noSE = _compute_noSE(T_SWEEP_K, T_SWEEP_K)             # u_s=0 fixed-bed reference at this GHSV's inlet fluxes
    ...
    eta     = elapsed/n_done*(n_ghsv - n_done)
    ...
    if res is not None:
        ...
        z_cross = _z_cross(res['z'], res['X_CO2'], X_TARGET)
        h2o_bal = _h2o_balance(u_s, res)
        z_str   = "n/a" if np.isnan(z_cross) else f"{z_cross:.3f} m"
        print(f"  GHSV={GHSV:.2f}  u_s={u_s*1e3:.4f} mm/s  Lambda={Lambda_check:.3f}  X={X_out:.1f}%  "
              f"q(0)={q_out:.3f}  T_max={T_max:.1f} C  z_cross={z_str}  "
        ...
        print(f"  GHSV={GHSV:.2f}  u_s={u_s*1e3:.4f} mm/s  Lambda={Lambda_check:.3f}  FAILED"
              f"  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
        z_cross = np.nan
        h2o_bal = None

    all_results[i_ghsv] = dict(res=res, u_s=u_s, GHSV=GHSV, Lambda_check=Lambda_check,
                                z_cross=z_cross, noSE=_noSE, h2o_balance=h2o_bal)

print(f"\nAll done.  Total: {_fmt_seconds(time.perf_counter() - t_run_start)}")

# ── Retry pass: high GHSV -> low GHSV, warm-starting from converged neighbours ──
print("\nRetry pass (backwards) for non-converged solutions...")
n_retried = 0
q_retry = None
for i_ghsv in reversed(range(n_ghsv)):
    e   = all_results.get(i_ghsv)
    if e is None:
        continue
    res = e['res']
    ...
    if res_new is not None:
        ...
        if improved:
            ...
            e['res']         = res_new
            e['z_cross']     = _z_cross(res_new['z'], res_new['X_CO2'], X_TARGET)
            e['h2o_balance'] = h2o_bal_new
        print(f"  RETRY GHSV={GHSV:.2f}  u_s={u_s*1e3:.2f} mm/s  [{tag}, {res_new['n_iter']} iter, "
              f"err={res_new['conv_err']:.2e}]  ({'kept' if improved else 'discarded'})  ({dt:.1f}s)")
        if e['h2o_balance'] is not None:
            print(_h2o_balance_line(e['h2o_balance']))
        q_retry = np.interp(np.linspace(0, L_b, 150), res_new['z'], res_new['q'])
        n_retried += 1
    else:
        print(f"  RETRY GHSV={GHSV:.2f}  u_s={u_s*1e3:.2f} mm/s  FAILED  ({dt:.1f}s)")

print(f"Retry pass done. {n_retried} solution(s) re-run.")
# endregion


# region 5. CONSOLE TABLE + REQUIRED-LENGTH SUMMARY
# =============================================================================
print(f"\nLength-determination table  (T={T_SWEEP_C:.0f} C, LAMBDA_TARGET={LAMBDA_TARGET:.2f}, "
      f"X_TARGET={X_TARGET:.1%})")
print(f"  {'GHSV':>6}  {'u_s [mm/s]':>10}  {'Lambda':>8}  {'z_cross [m]':>12}  {'iterations':>10}  {'converged':>9}")
for i_ghsv in range(n_ghsv):
    e   = all_results[i_ghsv]
    res = e['res']
    z_str    = 'not reached' if (res is None or np.isnan(e['z_cross'])) else f"{e['z_cross']:.3f}"
    it_str   = 'n/a' if res is None else f"{res['n_iter']}"
    conv_str = 'n/a' if res is None else ('y' if res['converged'] else 'n')
    print(f"  {e['GHSV']:>6.2f}  {e['u_s']*1e3:>10.4f}  {e['Lambda_check']:>8.3f}  {z_str:>12}  {it_str:>10}  {conv_str:>9}")

_reached = {i: all_results[i]['z_cross'] for i in range(n_ghsv) if not np.isnan(all_results[i]['z_cross'])}
if _reached:
    _i_worst    = max(_reached, key=_reached.get)
    _L_required = _reached[_i_worst]
    print(f"\nRequired reactor length (before margin): {_L_required:.3f} m  "
          f"(worst case at GHSV = {all_results[_i_worst]['GHSV']:.2f} m3_STP/(kg_ads·h))")
else:
    print(f"\nRequired reactor length (before margin): NOT DETERMINED -- "
          f"no swept GHSV reached X_TARGET={X_TARGET:.1%} within L_TRIAL={L_b:.2f} m")
# endregion
...
pal = plt.cm.plasma(np.linspace(0.1, 0.85, max(n_ghsv, 2)))

# ── Plot 0 (length determination): CO2 conversion vs z, one line per GHSV ───
# Kept as its own separate, early figure -- the actual deliverable of this sweep.
...
for i_ghsv in range(n_ghsv):
    ...
    lbl = f"GHSV={e['GHSV']:.2f} (u_s={e['u_s']*1e3:.2f} mm/s)" + ("" if res['converged'] else " (nc)")
    ax0.plot(res['z'], res['X_CO2']*100, color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
    if not np.isnan(e['z_cross']):
        ax0.plot(e['z_cross'], X_TARGET*100, marker='x', color='red', ms=10, mew=2.5, zorder=5)
ax0.axhline(X_TARGET*100, color='k', lw=1.2, ls=':', label=f'X_TARGET = {X_TARGET:.1%}')
ax0.set_xlabel('z [m]', fontsize=16);  ax0.set_ylabel('CO2 conversion [%]', fontsize=16)
ax0.tick_params(labelsize=13)
ax0.legend(fontsize=14);  ax0.grid(True, alpha=0.3);  ax0.set_ylim(0, 105)
plt.tight_layout()
_savefig('flux_plot0_length_determination_X_vs_z.png');  plt.show()

# ── Plot 1: Axial profiles ───────────────────────────────────────────────────
fig, axes = plt.subplots(2, 2, figsize=(16, 11))
fig.suptitle(f'MPB flux form  |  T = {T_SWEEP_C:.0f} C  |  U_a = {U_a:.0f} W/(m³·K), counter-current  |  '
             f'LAMBDA_TARGET = {LAMBDA_TARGET:.2f}', fontsize=16)
for i_ghsv in range(n_ghsv):
    e   = all_results[i_ghsv]
    res = e['res']
    if res is None:
    ...
    ls  = '-' if res['converged'] else '--'
    lbl = f"GHSV={e['GHSV']:.2f} (u_s={e['u_s']*1e3:.2f} mm/s)" + ("" if res['converged'] else " (nc)")
    axes[0,0].plot(res['z'], res['C_CO2']*1e3, color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
    axes[0,1].plot(res['z'], res['q'],          color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
    axes[1,0].plot(res['z'], res['X_CO2']*100,  color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
    axes[1,1].plot(res['z'], res['r']*1e3,      color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
...
for ax, (ylabel, title) in zip(axes.flat, labels_units):
    ...
    ax.set_title(title, fontsize=14);     ax.tick_params(labelsize=13)
...
plt.tight_layout()
_savefig(f'flux_plot1_axial_profiles_T{T_SWEEP_C:.0f}C.png');  plt.show()

# ── Plot 4: H2O profiles ─────────────────────────────────────────────────────
fig, (ax_q, ax_h) = plt.subplots(1, 2, figsize=(16, 5.5))
fig.suptitle(f'H2O profiles  |  T = {T_SWEEP_C:.0f} C  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=16)
for i_ghsv in range(n_ghsv):
    e   = all_results[i_ghsv]
    res = e['res']
    if res is None:
    ...
    ls  = '-' if res['converged'] else '--'
    lbl = f"GHSV={e['GHSV']:.2f} (u_s={e['u_s']*1e3:.2f} mm/s)" + ("" if res['converged'] else " (nc)")
    ax_q.plot(res['z'], res['q'], color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
    ax_h.plot(res['z'], res['C_H2O']*R_gas*res['T']/1e2, color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
ax_q.set_xlabel('z [m]', fontsize=16);  ax_q.set_ylabel('q [mol/kg]', fontsize=16)
ax_q.set_title('Solid H2O loading', fontsize=14);  ax_q.tick_params(labelsize=13)
ax_q.legend(fontsize=14);  ax_q.grid(True, alpha=0.3)
ax_h.set_xlabel('z [m]', fontsize=16);  ax_h.set_ylabel('p_H2O [mbar]', fontsize=16)
ax_h.set_title('Gas-phase H2O partial pressure', fontsize=14);  ax_h.tick_params(labelsize=13)
ax_h.legend(fontsize=14);  ax_h.grid(True, alpha=0.3)
plt.tight_layout()
_savefig(f'flux_plot4_H2O_profiles_T{T_SWEEP_C:.0f}C.png');  plt.show()

# ── Plot 5: Temperature profiles ─────────────────────────────────────────────
fig, ax_T = plt.subplots(figsize=(8, 5.5))
fig.suptitle(f'Temperature profile  |  T = {T_SWEEP_C:.0f} C  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=16)
for i_ghsv in range(n_ghsv):
    e   = all_results[i_ghsv]
    res = e['res']
    if res is None:
        continue
    regime = 'g' if res['gas_dominates'] else 's'
    ls     = '-' if res['converged'] else '--'
    lbl    = f"GHSV={e['GHSV']:.2f} (u_s={e['u_s']*1e3:.2f} mm/s, {regime})" + ("" if res['converged'] else " (nc)")
    ax_T.plot(res['z'], res['T'] - 273.15, color=pal[i_ghsv], lw=2, ls=ls, label=lbl)
ax_T.axhline(T_SWEEP_C, color='grey', lw=1.5, ls='--', alpha=0.8, label=f'T_in = T_wall = {T_SWEEP_C:.0f} °C')
ax_T.set_xlabel('z [m]', fontsize=16);  ax_T.set_ylabel('T [°C]', fontsize=16)
ax_T.set_title('(g) = gas-dominated  |  (s) = solid-dominated (T from solid IVP)', fontsize=14)
ax_T.tick_params(labelsize=13)
ax_T.legend(fontsize=14);  ax_T.grid(True, alpha=0.3)
plt.tight_layout()
_savefig(f'flux_plot5_temperature_T{T_SWEEP_C:.0f}C.png');  plt.show()

# ── Plot 7: H2O budget decomposition (reaction vs sorbent rates) ────────────
fig, (ax_rate, ax_cumul) = plt.subplots(1, 2, figsize=(16, 5.5))
fig.suptitle(f'H2O budget  |  T = {T_SWEEP_C:.0f} C  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=16)
...
for i_ghsv in range(n_ghsv):
    ...
    if res is None:
        continue
    ls  = '-' if res['converged'] else '--'
    lbl = f"GHSV={e['GHSV']:.2f}" + ("" if res['converged'] else " (nc)")

    p_H2O_f = res['C_H2O'] * R_gas * res['T'] / 1e5   # [bar]
    qs_f    = q_star(res['T'], p_H2O_f)
    Kl_f    = K_LDF(res['T'], p_H2O_f)
    ads_f   = Kl_f * (qs_f - res['q'])               # [mol/(kg_ads·s)]

    S_rxn = 2.0 * rho_bed_cat * res['r']             # [mol/(m³_bed·s)]
    S_ads = -rho_bed_ads * ads_f                      # [mol/(m³_bed·s)]

    F_rxn = cumulative_trapezoid(S_rxn, res['z'], initial=0)
    F_ads = cumulative_trapezoid(S_ads, res['z'], initial=0)
    F_net = F_rxn + F_ads

    ax_rate.plot(res['z'], S_rxn*1e3, color=pal[i_ghsv], lw=2,   ls=ls,            label=lbl)
    ax_rate.plot(res['z'], S_ads*1e3, color=pal[i_ghsv], lw=1.5, ls=ls, alpha=0.5)

    ax_cumul.plot(res['z'], F_rxn*1e3, color=pal[i_ghsv], lw=2,   ls=ls,            label=f"{lbl} rxn")
    ax_cumul.plot(res['z'], F_ads*1e3, color=pal[i_ghsv], lw=1.5, ls=ls, alpha=0.5, label=f"{lbl} ads")
    ax_cumul.plot(res['z'], F_net*1e3, color=pal[i_ghsv], lw=1,   ls=ls, alpha=0.3)

ax_rate.axhline(0, color='k', lw=0.8, ls=':')
ax_rate.set_xlabel('z [m]', fontsize=16);  ax_rate.set_ylabel('S_H2O [mmol/(m³_bed·s)]', fontsize=16)
ax_rate.set_title('Local H2O source (+) / sink (−) rates\nsolid line = rxn,  faint = sorbent', fontsize=14)
