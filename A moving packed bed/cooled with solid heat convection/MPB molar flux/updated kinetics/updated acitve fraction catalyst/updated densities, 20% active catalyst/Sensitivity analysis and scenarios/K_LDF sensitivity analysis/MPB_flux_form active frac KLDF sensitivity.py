"""
Moving Packed Bed (MPB) Reactor Model — K_LDF Sensitivity Analysis
==========================================================================================
Same single operating point as the Sec. 7.1 reference-case deep dive
("MPB_flux_form_7.1_reference_case.py"):
    u_s   = 5.84 mm/s
    GHSV  = 1.5 m3_STP/(kg_ads.h)
    T_in  = T_wall = 280 C
    dilution = script default (y_CH4_in = 0.80)

Instead of a single K_LDF, the LDF mass-transfer coefficient is scaled by a
uniform multiplier and solved three times:
    KLDF_MULTIPLIERS = [0.1, 1.0, 10.0]   (0.1x / 1x baseline / 10x)
...
"MPB_flux_form_7.1_reference_case.py", except:
  - K_LDF() now multiplies its return value by a module-level KLDF_MULTIPLIER.
    solve_mpb itself is NOT modified: every internal rate/adsorption
    evaluation already calls K_LDF(), so setting KLDF_MULTIPLIER once before
    a solve_mpb call makes that whole solve use the scaled coefficient.
  - region 5 solves the reference case once per KLDF_MULTIPLIERS entry
    (instead of once), storing every per-case diagnostic needed for the
    figures immediately after that case's solve (while KLDF_MULTIPLIER is
    still set correctly for it).
  - region 6 remakes all 8 of the reference-case figures, each now overlaying
    the three K_LDF cases (one colour per multiplier) instead of a single
...
# --- Reference-case operating point (held fixed across the K_LDF sweep) ---
u_s_case = 0.00584   # [m/s]  solid (sorbent/catalyst) velocity
T_C_case = 280       # [C]    inlet temperature

# --- K_LDF sensitivity sweep ---
KLDF_MULTIPLIERS = [1.0, 0.2, 0.1, 0.04, 0.02]   # [-]  uniform multipliers applied to K_LDF
KLDF_COLORS = {1.0: 'tab:purple', 0.2: 'tab:blue', 0.1: 'tab:green', 0.04: 'tab:orange', 0.02: 'tab:red'}
KLDF_LABELS = {1.0: '1x $K_{LDF}$ (baseline)', 0.2: '0.2x $K_{LDF}$', 0.1: '0.1x $K_{LDF}$',
               0.04: '0.04x $K_{LDF}$', 0.02: '0.02x $K_{LDF}$'}
KLDF_MULTIPLIER = 1.0   # [-]  active multiplier; K_LDF() reads this module-level global on every call, set per-case in region 5

print(f"MPB flux form — K_LDF sensitivity analysis: d={d_b*100:.0f} cm, L={L_b:.1f} m, "
...
print(f"  K_LDF multipliers swept: {KLDF_MULTIPLIERS}")
# endregion
...
def K_LDF(T_K, p_H2O):
    """LDF mass-transfer coefficient, scaled by the active KLDF_MULTIPLIER
    (module-level global, set per-case in region 5 below). solve_mpb calls
    this function directly, so it always sees whichever multiplier is
    currently active — no changes needed inside solve_mpb itself."""
    return K_LDF_vec(T_K, p_H2O, W0_DA, E_DA, n_DA) * KLDF_MULTIPLIER
...
def solve_mpb(u_s, T_K, T_wall=None, max_iter=1000, tol=1e-5, N=400, q_init=None):
    ...
    evaluation below calls K_LDF(), which applies the module-level
    KLDF_MULTIPLIER — so scaling K_LDF for a whole solve is done purely by
...
def _compute_noSE(T_K, T_wall, N=300):
    ...
    (noSE). No adsorption term at all, so it does NOT depend on K_LDF /
    KLDF_MULTIPLIER — computed once and reused across all three K_LDF cases."""
    def rhs(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
        F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
        T_l     = max(y[4], 200.0)
...
def _q_physics_init(T_K, N=150):
    ...
    guess for warm-starting solve_mpb. Uses q_star only (no K_LDF call), so
    it is also independent of KLDF_MULTIPLIER and computed once for all
    three cases."""
    def rhs_noads(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
        F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
...
# region 5. SOLVE EACH K_LDF CASE
# =============================================================================
T_K_case    = T_C_case + 273.15
T_wall_case = T_K_case   # adiabatic wall (T_wall = T_in), same convention as the reference-case script

print(f"\n{'='*70}")
print(f"  K_LDF sensitivity: u_s = {u_s_case*1e3:.2f} mm/s, GHSV = {GHSV:.1f}, "
      f"T_in = T_wall = {T_C_case} C")
print(f"{'='*70}")

noSE = _compute_noSE(T_K_case, T_wall_case)          # independent of K_LDF -- computed once
X_eq = equilibrium_conversion(T_K_case)               # thermodynamic, independent of K_LDF
...
_phys       = _q_physics_init(T_K_case)               # independent of K_LDF -- computed once
q_init_base = _phys['q'][::-1]                        # flip: solid enters unloaded at z=L, builds up toward z=0

cases = {}   # keyed by K_LDF multiplier -> dict of every per-case diagnostic needed downstream

for mult in KLDF_MULTIPLIERS:
    KLDF_MULTIPLIER = mult   # module-level assignment (top-level script scope): K_LDF() reads this on every call inside solve_mpb

    print(f"\n{'-'*70}")
    print(f"  Solving K_LDF case: {KLDF_LABELS[mult]}  (multiplier = {mult:g}x)")
    print(f"{'-'*70}")

    t0  = time.perf_counter()
    res = solve_mpb(u_s_case, T_K_case, T_wall=T_wall_case, q_init=q_init_base)
    attempt = 1
    while (res is None or not res['converged']) and attempt < 5:
        ...
        res = solve_mpb(u_s_case, T_K_case, T_wall=T_wall_case, q_init=q_retry,
                         max_iter=2000, tol=1e-6)
    dt = time.perf_counter() - t0

    if res is None:
        raise RuntimeError(f"solve_mpb failed to produce a solution for K_LDF multiplier {mult:g}x.")
    ...
    #    NOW while KLDF_MULTIPLIER is still correctly set for this case ──
    ...
    # ── requested: peak p_H2O along the bed, and the maximal local
    #    desorption ("regeneration") rate -- the most negative point of
    #    ads_f = K_LDF*(q*-q); negative means q > q* (desorption). If ads_f
    #    is never negative, the bed is net-adsorbing everywhere and the
    #    regeneration rate is reported as 0. ──
    p_H2O_max = float(p_H2O_z[i_pmax])   # [bar]
    q_outlet  = float(res['q'][0])       # [mol/kg_ads], solid loading at its outlet (z=0, where the solid leaves)
    i_regen        = int(np.argmin(ads_f))
    ads_regen_max  = float(min(ads_f[i_regen], 0.0))   # [mol/(kg_ads.s)], <=0
    z_regen        = float(z[i_regen])
    ...
    if ads_regen_max < 0.0:
        print(f"    max local desorption (regeneration) rate = {-ads_regen_max*1e3:.4f} mmol/(kg_ads.s)  "
              f"(z={z_regen:.3f} m)")
    else:
        print("    max local desorption (regeneration) rate = 0.0000 mmol/(kg_ads.s)  "
              "(no desorption zone: bed is net-adsorbing everywhere)")
    ...
    cases[mult] = dict(
        ...
        p_H2O_max=p_H2O_max, q_outlet=q_outlet, ads_regen_max=ads_regen_max, z_regen=z_regen,
    )

KLDF_MULTIPLIER = 1.0   # restore baseline after the sweep (harmless -- nothing below calls solve_mpb again)
print(f"\nAll {len(KLDF_MULTIPLIERS)} K_LDF cases solved.")
# endregion
...
def _savefig(filename):
    ...
    filename  = f'{stem}_KLDF_sensitivity{ext}'
...
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    ax.plot(c['z'], c['res']['X_CO2']*100, color=KLDF_COLORS[mult], lw=2.5, label=KLDF_LABELS[mult])
...
ax.set_title(f'CO2 conversion vs bed position — $K_{{LDF}}$ sensitivity  |  $T_{{in}}$={T_C_case} C')
ax.legend(fontsize=10);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('sec7_1_fig1_conversion_vs_z.png');  plt.show()

# ── Figure 2: Gas-phase composition vs z (one panel per K_LDF case) ─────────
fig, axes = plt.subplots(1, 3, figsize=(16, 5.5), sharey=False)
for ax1, mult in zip(axes, KLDF_MULTIPLIERS):
    r = cases[mult]['res']
    ...
    ax1.set_title(KLDF_LABELS[mult], fontsize=10)
    ax1.grid(True, alpha=0.3)

    if mult == KLDF_MULTIPLIERS[-1]:
        lines1, labels1 = ax1.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        fig.legend(lines1+lines2, labels1+labels2, fontsize=9, loc='upper center',
                   bbox_to_anchor=(0.5, 0.02), ncol=4)
fig.suptitle('Gas-phase composition vs z — $K_{LDF}$ sensitivity', fontsize=11)
...
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    ax.plot(c['z'], c['res']['q'], color=KLDF_COLORS[mult], lw=2.5,
            label=f"{KLDF_LABELS[mult]} — q(z)")
    ax.plot(c['z'], c['q_eq_z'],   color=KLDF_COLORS[mult], lw=1.5, ls='--', alpha=0.6,
            label=f"{KLDF_LABELS[mult]} — q*(z)")
ax.set_xlabel('z [m]');  ax.set_ylabel('H2O loading [mol/kg_ads]')
ax.set_title('Solid H2O loading vs equilibrium — $K_{LDF}$ sensitivity')
...
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    ax.plot(c['z'], c['res']['T']-273.15, color=KLDF_COLORS[mult], lw=2.5, label=KLDF_LABELS[mult])
    i_max = int(np.argmax(c['res']['T']))
    ax.plot(c['z'][i_max], c['res']['T'][i_max]-273.15, 'o', color=KLDF_COLORS[mult], ms=6)
ax.axhline(T_C_case, color='grey', lw=1.5, ls='--', label=f'$T_{{in}}$ = $T_{{wall}}$ = {T_C_case} C')
ax.set_xlabel('z [m]');  ax.set_ylabel('T [C]')
ax.set_title('Temperature profile — $K_{LDF}$ sensitivity')
...
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    ax1.plot(c['z'], c['res']['r']*1e3, color=KLDF_COLORS[mult], lw=1.5, alpha=0.6,
             label=f"{KLDF_LABELS[mult]} — r(z)")
ax1.set_xlabel('z [m]');  ax1.set_ylabel('r [mmol/(kg_cat·s)]')

ax2 = ax1.twinx()
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    ax2.plot(c['z'], c['res']['X_CO2']*100, color=KLDF_COLORS[mult], lw=2.5,
             label=f"{KLDF_LABELS[mult]} — $X_{{CO2}}$(z)")
...
ax1.set_title('Local reaction rate and cumulative conversion — $K_{LDF}$ sensitivity')
ax1.grid(True, alpha=0.3)
plt.tight_layout()
...
for axA, mult in zip(axesA, KLDF_MULTIPLIERS):
    c = cases[mult];  z = c['z']
    ...
    axA.set_title(KLDF_LABELS[mult], fontsize=10)
...
fig.suptitle(f'Energy budget: local generation/removal terms — $K_{{LDF}}$ sensitivity  |  $T_{{in}}$={T_C_case} C',
             fontsize=11)
...
for mult in KLDF_MULTIPLIERS:
    c = cases[mult]
    axB.plot(c['z'], c['C_gas'],   color=KLDF_COLORS[mult], lw=2.2,
             label=f"{KLDF_LABELS[mult]} — gas")
    axB.plot(c['z'], c['C_solid'], color=KLDF_COLORS[mult], lw=2.2, ls='--',
             label=f"{KLDF_LABELS[mult]} — solid")
axB.axhline(0, color='k', lw=0.8, ls=':')
axB.set_xlabel('z [m]');  axB.set_ylabel('cumulative sensible heat picked up [W]')
axB.set_title('Cumulative sensible heat carried by gas vs. solid — $K_{LDF}$ sensitivity', fontsize=10)
...
n_cases = len(KLDF_MULTIPLIERS)
group_x = np.arange(n_cases)
bar_w   = 0.32

for i, mult in enumerate(KLDF_MULTIPLIERS):
    c = cases[mult]
...
ax.set_xticks(group_x, [f"{KLDF_LABELS[m]}\n(mismatch {cases[m]['mismatch_bb']:+.1f}%)" for m in KLDF_MULTIPLIERS],
              fontsize=8.5)
ax.set_ylabel('heat rate [W]')
ax.axhline(0, color='k', lw=0.8)
ax.set_title(f'Whole-bed heat balance — $K_{{LDF}}$ sensitivity  |  $T_{{in}}$={T_C_case} C', fontsize=11)
...
# =============================================================================
def _z_at_conversion(res, X_target=0.95):
    """z [m] at which X_CO2 first reaches X_target (linear interp on the
    z-X_CO2 profile). Returns None if the outlet conversion never reaches it."""
    z, X = res['z'], res['X_CO2']
    if X[-1] < X_target:
        return None
    return float(np.interp(X_target, X, z))

print(f"\n{'='*90}")
print("  K_LDF sensitivity — comparison table")
print(f"{'='*90}")
header = (f"  {'case':<26}{'X_out[%]':>10}{'z_X95[m]':>10}{'T_max[C]':>10}{'q_out':>9}"
          f"{'p_H2O_max[mbar]':>17}{'regen[mmol/kg.s]':>18}"
          f"{'H2O err[%]':>12}{'E closure[%]':>14}{'tau_gas[s]':>12}{'1/K_LDF_pk[s]':>15}")
print(header)
for mult in KLDF_MULTIPLIERS:
    c   = cases[mult];  r = c['res']
    X_out = float(r['X_CO2'][-1])*100
    z_X95 = _z_at_conversion(r, 0.95)
    z_X95_str = f"{z_X95:10.3f}" if z_X95 is not None else f"{'n/a':>10}"
    T_max = float(np.max(r['T']))-273.15
    q_out = float(r['q'][0])
    print(f"  {KLDF_LABELS[mult]:<26}{X_out:10.2f}{z_X95_str}{T_max:10.1f}{q_out:9.3f}"
          f"{c['p_H2O_max']*1e3:17.2f}{-c['ads_regen_max']*1e3:18.4f}"
