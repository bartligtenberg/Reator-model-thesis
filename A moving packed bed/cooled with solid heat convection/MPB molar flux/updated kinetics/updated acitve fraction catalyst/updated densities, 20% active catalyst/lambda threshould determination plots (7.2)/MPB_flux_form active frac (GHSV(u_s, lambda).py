...
for T_C in T_IN_LIST:                                               # outer sweep: one pass per inlet temperature in T_IN_LIST (currently just [280] C)
    ...
    for i_us, u_s in enumerate(U_S_LIST):                             # inner sweep: one solve_mpb call per solid velocity in U_S_LIST, for this T_C
...
# ── Lambda calibration: q*_max reference & sorbent-utilisation table ────────
# p_H2O_max_bound: analytic upper bound on gas-phase water partial pressure, reached
# at full CO2 conversion (Sabatier stoichiometry: 1 mol CO2 -> 2 mol H2O, net -2 mol
# gas per mol CO2 reacted). Same formula used for Lambda_thermo in the GHSV heatmap
# sweep script ("heatmap GHSV, u_s and z coordinate for 99.5conv/"); q_star_max is
# computed once here and reused for both the plot below and the printed table, so
# the two report an identical ceiling loading.
p_H2O_max_bound = P_bar * (2.0*F_in_CO2) / (F_in_CH4 + F_in_CO2 + 2.0*F_in_CO2)   # [bar]
q_star_max = float(q_star(T_K_prof, np.array([p_H2O_max_bound]))[0])              # [mol/kg]  DA ceiling loading at this p_H2O bound

# ── Plot 8: q(z) profiles vs u_s, with q*_max reference ──────────────────────
fig, ax_q8 = plt.subplots(figsize=(9, 5))
pal_q8 = plt.cm.plasma(np.linspace(0.1, 0.85, len(U_S_LIST)))
for k, u_s in enumerate(U_S_LIST):
    e = all_results.get((T_C_PROF, k))
    if e is None or e['res'] is None:
        continue
    r   = e['res']
    ls  = '-' if r['converged'] else '--'
    lbl = f"u_s = {u_s*1e3:.2f} mm/s" + ("" if r['converged'] else " (nc)")
    ax_q8.plot(r['z'], r['q'], color=pal_q8[k], lw=2, ls=ls, label=lbl)
ax_q8.axhline(q_star_max, color='k', lw=1.5, ls='--')
ax_q8.text(0.02*L_b, q_star_max, f'q*_max = {q_star_max:.3f} mol/kg',
           color='k', fontsize=9, va='bottom', ha='left')
ax_q8.set_xlabel('z [m]', fontsize=11);  ax_q8.set_ylabel('q [mol/kg_ads]', fontsize=11)
ax_q8.set_title(f'MPB flux form  |  GHSV = {GHSV:.2f} m³_STP/(kg_ads·h)  |  T_in = {T_C_PROF} C  '
                 f'— Sorbent loading profiles', fontsize=10)
ax_q8.legend(fontsize=8);  ax_q8.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('flux_plot8_q_profiles_lambda_calibration.png');  plt.show()

# ── Console: Lambda / utilisation table (for hand-copy into a LaTeX table) ──
print(f"\nLambda calibration table  (T_in={T_C_PROF} C, GHSV={GHSV:.2f} m³_STP/(kg_ads·h), "
      f"q*_max={q_star_max:.4f} mol/kg @ p_H2O_max_bound={p_H2O_max_bound*1e3:.2f} mbar)")
print(f"  {'u_s [mm/s]':>10}  {'Lambda':>8}  {'q(0) [mol/kg]':>14}  {'utilisation':>12}  {'iterations':>10}")
for i_us, u_s in enumerate(U_S_LIST):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        print(f"  {u_s*1e3:>10.3f}  {'--':>8}  {'--':>14}  {'--':>12}  {'--':>10}")
        continue
    m        = get_metrics(e)
    Lambda_i = (u_s * rho_bed_ads * q_star_max) / (2.0 * F_in_CO2)   # corrected Lambda, using the same q_star_max as the plot above
    q0       = m['q_out']
    util     = q0 / q_star_max
    n_iter   = e['res']['n_iter']
    print(f"  {u_s*1e3:>10.3f}  {Lambda_i:>8.3f}  {q0:>14.4f}  {util:>12.3f}  {n_iter:>10d}")
# endregion
