...
# --- MPB scan parameters ---
U_S_LIST  = np.array([8.0, 6.0, 5.0, 4.0, 3.0]) * 1e-3
T_IN_LIST = [280]
...
# region 4b. ENERGY BALANCE CHECK PLOTS (per u_s)
# =============================================================================
# For each converged (T_C, u_s) case, reconstruct the local energy-balance terms
# (Q_rxn, Q_ads, Q_wall) and adsorption rate from the solved r(z)/q(z) profiles,
# then save a 2x2 diagnostic figure per case: LHS vs RHS of the energy ODE,
# cumulative energy budget, local source terms, and adsorption loading.
# Mirrors the panels in "old kinetics (assumed 5% active fraction)/Energy balance
# check/energy_balance_check.py". solve_mpb here only returns concentrations (not
# raw molar fluxes), so F_i(z) is reconstructed from r(z)/q(z) via the same
# cumulative stoichiometric integrals used in that reference script.
_EB_SAVE_DIR = os.path.dirname(os.path.abspath(__file__))
_EB_TAG      = f"active_frac_{active_fraction:.0%}"

def _make_energy_balance_plot(entry, T_C):
    res = entry['res']
    ...
    z = res['z'];  T = res['T'];  q = res['q'];  r = res['r']

    # Solid ODE: dq/dzeta = ads/u_s with zeta = L-z  ->  ads = -u_s*dq/dz
    # (numerically stable in the equilibrium limit, avoids Kl*(qs-q) cancellation)
    ads = -u_s * np.gradient(q, z)

    n_rxn_cum = cumulative_trapezoid(rho_bed_cat * r,   z, initial=0.0)
    n_ads_cum = cumulative_trapezoid(rho_bed_ads * ads, z, initial=0.0)
    F_CO2_z = F_in_CO2 - n_rxn_cum
    F_H2_z  = F_in_H2  - 4.0*n_rxn_cum
    F_CH4_z = F_in_CH4 + n_rxn_cum
    F_H2O_z = 2.0*n_rxn_cum - n_ads_cum   # F_in_H2O = 0 (dry feed)

    gas_cap_prof  = (F_CO2_z*Cp_CO2 + F_H2_z*Cp_H2 + F_CH4_z*Cp_CH4 + F_H2O_z*Cp_H2O)
    solid_cap_val = u_s * rho_bed_tot * Cp_cat

    p_H2O   = np.clip(F_H2O_z / np.maximum(F_CO2_z+F_H2_z+F_CH4_z+F_H2O_z, 1e-30) * P_bar, 0.0, None)
    ...
    fig.suptitle(
        f'Energy Balance Check  |  u_s = {u_s*1e3:.2f} mm/s  |  T_in = {T_C} °C  |  '
        f'{regime}  |  U_a = {U_a:.0f} W/(m³·K)  |  {_EB_TAG}', fontsize=11)
    ...
    ax.plot(z, residual, lw=1.5, color='red', alpha=0.8, label='Residual (LHS−RHS)')
    ax.axhline(0, color='grey', lw=0.8, ls=':')
    ax.set_xlabel('z [m]');  ax.set_ylabel('[W/m³]')
    ax.set_title('Energy ODE: LHS vs RHS  (should overlap)')
    ax.legend(fontsize=8);  ax.grid(True, alpha=0.3)

    # Panel 2 -- cumulative energy budget along z
    ax = axes[0, 1]
    ...
    gas_conv_cum   = cumulative_trapezoid(gas_cap_prof*dTdz,    z, initial=0.0)
    solid_conv_cum = cumulative_trapezoid(-solid_cap_val*dTdz, z, initial=0.0)
    ...
    ax.set_title('Adsorption loading')
    ax.legend(loc='upper left',  fontsize=8)
    ax2.legend(loc='upper right', fontsize=8)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    fname = f'energy_balance_check_us{u_s*1e3:.2f}mms_T{T_C}C_{_EB_TAG}.png'
    plt.savefig(os.path.join(_EB_SAVE_DIR, fname), dpi=150, bbox_inches='tight')
    plt.close(fig)

for (_T_C, _i_us), _entry in all_results.items():
    _make_energy_balance_plot(_entry, _T_C)
# endregion
