...
# --- MPB scan parameters ---
U_S_LIST  = np.array([1, 2, 3, 4, 6, 8, 10]) * 1e-3
T_IN_LIST = [280]
...
# =============================================================================
def solve_mpb(u_s, T_K, T_wall=None, max_iter=600, tol=1e-5, N=100, q_init=None,
              refine_ends=False):
    """
    ...
    if refine_ends:
        ...
        z_grid = np.unique(np.concatenate([
            np.linspace(0.0,         _z_end,      _n_end),
            np.linspace(_z_end,      L_b-_z_end,  _n_mid),
            np.linspace(L_b-_z_end, L_b,          _n_end),
        ]))
    else:
...
_GRID_CONFIGS = [
    ('N=50  uniform', dict(N=50,  refine_ends=False)),
    ('N=100 uniform', dict(N=100, refine_ends=False)),
    ('N=200 uniform', dict(N=200, refine_ends=False)),
    ('N=400 uniform', dict(N=400, refine_ends=False)),
    ('N=600 uniform', dict(N=600, refine_ends=False)),
    ('N=800  uniform', dict(N=800, refine_ends=True)),
    ('N=1000 uniform', dict(N=1000, refine_ends=True))
]

all_results_grids = {name: {} for name, _ in _GRID_CONFIGS}
noSE_results = {}
...
n_total     = len(_GRID_CONFIGS) * len(T_IN_LIST) * len(U_S_LIST)
n_done      = 0
t_run_start = time.perf_counter()

for cfg_name, cfg_kwargs in _GRID_CONFIGS:
    print(f"\n{'='*70}")
    print(f"  GRID CONFIG: {cfg_name}")
    print(f"{'='*70}")
    ...
    for T_C in T_IN_LIST:
        ...
        for i_us, u_s in enumerate(U_S_LIST):
            ...
            if res is not None:
                ...
                # H2O balance: produced by rxn = gas-phase exit + adsorbed by solid
                H2O_rxn = 2.0 * dCO2
                H2O_gas = res['F_H2O_out']
                H2O_ads = q_out * rho_bed_ads * u_s
                H2O_err = (H2O_gas + H2O_ads) / max(H2O_rxn, 1e-15) - 1.0
                print(f"  u_s={u_s*1e3:.2f} mm/s  X={X_out:.1f}%  "
                      f"q(0)={q_out:.3f}  q_sat={sat:.2f}{flag}  "
                      f"CH4/CO2={s_C:.3f}  H2/CO2={s_H:.3f}  "
                      f"H2O_bal={H2O_err*100:+.1f}%  "
                      f"T_max={T_max:.1f}C  "
            ...
            all_results_grids[cfg_name][(T_C, i_us)] = {
                'res': res, 'u_s': u_s, 'T_K': T_K, 'T_wall': T_wall}

print(f"\nAll done.  Total: {_fmt_seconds(time.perf_counter() - t_run_start)}")

# N=100 uniform used as baseline for existing plots
all_results = all_results_grids['N=100 uniform']
# endregion
...
plt.tight_layout()
_savefig(f'flux_plot1_axial_profiles_T{T_C_PROF}C.png');  plt.close()
...
plt.tight_layout()
_savefig('flux_plot2_conversion_vs_us.png');  plt.close()
...
plt.tight_layout()
_savefig('flux_plot3_sorbent_utilisation.png');  plt.close()
...
plt.tight_layout()
_savefig(f'flux_plot4_H2O_profiles_T{T_C_PROF}C.png');  plt.close()
...
plt.tight_layout()
_savefig(f'flux_plot5_temperature_T{T_C_PROF}C.png');  plt.close()
...
plt.tight_layout()
_savefig('flux_plot6_Tmax_vs_us.png');  plt.close()

# ── Plot 7: Grid convergence comparison ──────────────────────────────────────
_cfg_colors = ['lightcoral', 'steelblue', 'darkorange', 'seagreen', 'mediumorchid', 'purple']
fig7, (ax7X, ax7q, ax7T, ax7H, ax7H2) = plt.subplots(1, 5, figsize=(27, 5))
fig7.suptitle(f'Grid convergence  |  T_in = {T_C_PROF} °C  |  U_a = {U_a:.0f} W/(m³·K)',
              fontsize=12)
for (cfg_name, _), col in zip(_GRID_CONFIGS, _cfg_colors):
    us_vals, X_vals, q_vals, T_vals, H_vals, H2_vals = [], [], [], [], [], []
    for i_us, u_s in enumerate(U_S_LIST):
        e = all_results_grids[cfg_name].get((T_C_PROF, i_us))
        if e and e['res'] is not None:
            r = e['res']
            dCO2    = F_in_CO2 - r['F_CO2_out']
            H2O_rxn = 2.0 * dCO2
            H2O_gas = r['F_H2O_out']
            H2O_ads = float(r['q'][0]) * rho_bed_ads * u_s
            H2O_err = (H2O_gas + H2O_ads) / max(H2O_rxn, 1e-15) - 1.0
            us_vals.append(u_s * 1e3)
            X_vals.append(float(r['X_CO2'][-1]) * 100)
            q_vals.append(float(r['q'][0]))
            T_vals.append(float(np.max(r['T'])) - 273.15)
            H_vals.append(H2O_err * 100)
            H2_vals.append(r['F_H2_out'] / F_in_H2 * 100)
    if us_vals:
        ...
        ax7H.plot(us_vals, H_vals,  marker='o', lw=2, ms=5, color=col, label=cfg_name)
        ax7H2.plot(us_vals, H2_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
ax7X.set_xlabel('u_s [mm/s]', fontsize=11);  ax7X.set_ylabel('CO2 conversion [%]', fontsize=11)
ax7X.set_title('X_CO2 at exit (z=L)', fontsize=10)
ax7X.legend(fontsize=9);  ax7X.grid(True, alpha=0.3)
ax7q.set_xlabel('u_s [mm/s]', fontsize=11);  ax7q.set_ylabel('q(z=0) [mol/kg]', fontsize=11)
ax7q.set_title('Solid H2O loading at solid exit (z=0)', fontsize=10)
ax7q.legend(fontsize=9);  ax7q.grid(True, alpha=0.3)
ax7T.set_xlabel('u_s [mm/s]', fontsize=11);  ax7T.set_ylabel('T_max [°C]', fontsize=11)
ax7T.set_title('Peak temperature', fontsize=10)
ax7T.legend(fontsize=9);  ax7T.grid(True, alpha=0.3)
ax7H.axhline(0, color='grey', lw=1.5, ls='--', alpha=0.7)
ax7H.set_xlabel('u_s [mm/s]', fontsize=11)
ax7H.set_ylabel('H2O balance error  (gas+ads)/rxn − 1  [%]', fontsize=11)
ax7H.set_title('H2O balance error', fontsize=10)
ax7H.legend(fontsize=9);  ax7H.grid(True, alpha=0.3)
ax7H2.set_xlabel('u_s [mm/s]', fontsize=11)
ax7H2.set_ylabel('H2 at outlet  F_H2_out / F_H2_in  [%]', fontsize=11)
ax7H2.set_title('H2 remaining at gas outlet (z=L)', fontsize=10)
ax7H2.legend(fontsize=9);  ax7H2.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('flux_plot7_grid_convergence.png');  plt.close()
# endregion
