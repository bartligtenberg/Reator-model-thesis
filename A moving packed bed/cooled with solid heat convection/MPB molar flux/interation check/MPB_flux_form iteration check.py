...
# --- MPB scan parameters ---
U_S_LIST  = np.array([0.5, 1, 1.5, 2, 3, 4, 6, 8, 10]) * 1e-3
T_IN_LIST = [280]

_N_DEFAULT = 200   # default grid points passed to solve_mpb
...
print(f"  d_p = {d_p*1e3:.2f} mm  |  N (grid points) = {_N_DEFAULT}")
# endregion
...
# =============================================================================
def solve_mpb(u_s, T_K, T_wall=None, max_iter=200, tol=1e-5, N=200, q_init=None):
    """
...
# tol=0.0 forces solver to always run exactly max_iter iterations (never stops early),
# so comparing configs directly shows how the solution changes with more iterations.
_ITER_CONFIGS = [
    ('iter=25',   dict(max_iter=25,   tol=1e-4)),
    ('iter=50',   dict(max_iter=50,   tol=1e-4)),
    ('iter=100',  dict(max_iter=100,  tol=1e-4)),
    ('iter=200',  dict(max_iter=200,  tol=1e-4)),
    ('iter=400',  dict(max_iter=400,  tol=1e-4)),
    ('iter=600',  dict(max_iter=600,  tol=1e-4)),
    ('iter=800',  dict(max_iter=800,  tol=1e-4)),
    ('iter=1000', dict(max_iter=1000, tol=1e-4)),
]

all_results_iters = {name: {} for name, _ in _ITER_CONFIGS}
noSE_results = {}

for T_C in T_IN_LIST:
    T_K = T_C + 273.15;  T_wall = T_K
    noSE_results[T_C] = _compute_noSE(T_K, T_wall)

gas_cap_in = _gas_cap(F_in_CO2, F_in_H2, F_in_CH4, 0.0)
u_s_star   = gas_cap_in / (rho_bed_tot * Cp_cat)
print(f"\n  u_s* = {u_s_star*1e3:.3f} mm/s  |  non-SE X = "
      f"{noSE_results[T_IN_LIST[0]]['X_CO2_noSE']*100:.1f}%")

n_total     = len(_ITER_CONFIGS) * len(T_IN_LIST) * len(U_S_LIST)
n_done      = 0
t_run_start = time.perf_counter()

for cfg_name, cfg_kwargs in _ITER_CONFIGS:
    print(f"\n{'='*70}")
    print(f"  ITER CONFIG: {cfg_name}")
    print(f"{'='*70}")
    ...
    for T_C in T_IN_LIST:
        ...
        for i_us, u_s in enumerate(U_S_LIST):
            ...
            res = solve_mpb(u_s, T_K, T_wall=T_wall, q_init=q_init, **cfg_kwargs)
            ...
            if res is not None:
                ...
                # Stoichiometry: CH4 produced / CO2 consumed ≈ 1; H2 consumed / CO2 consumed ≈ 4
                dCO2 = F_in_CO2 - res['F_CO2_out']
                dCH4 = res['F_CH4_out'] - F_in_CH4
                dH2  = F_in_H2  - res['F_H2_out']
                s_C  = dCH4 / max(dCO2, 1e-15)
                s_H  = dH2  / max(dCO2, 1e-15)
                # H2O balance: produced = out_gas + out_solid
                H2O_prod = 2.0 * dCO2
                H2O_gas  = res['F_H2O_out']
                H2O_sol  = u_s * rho_bed_ads * q_out
                H2O_bal  = (H2O_gas + H2O_sol) / max(H2O_prod, 1e-20) * 100
                print(f"  u_s={u_s*1e3:.2f} mm/s  X={X_out:.1f}%  "
                      f"q(0)={q_out:.3f}  "
                      f"H2O: prod={H2O_prod*1e4:.3f}  gas={H2O_gas*1e4:.3f}  sol={H2O_sol*1e4:.3f}  bal={H2O_bal:.1f}%  "
                      f"CH4/CO2={s_C:.3f}  H2/CO2={s_H:.3f}  "
                      f"T_max={T_max:.1f}C  "
                      f"[{regime}-dom, err={res['conv_err']:.2e}]"
                      f"  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
                q_init = np.interp(np.linspace(0, L_b, 150), res['z'], res['q'])
            else:
                print(f"  u_s={u_s*1e3:.2f} mm/s  FAILED  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
            all_results_iters[cfg_name][(T_C, i_us)] = {
                'res': res, 'u_s': u_s, 'T_K': T_K, 'T_wall': T_wall}

print(f"\nAll done.  Total: {_fmt_seconds(time.perf_counter() - t_run_start)}")

# iter=1000 used as baseline for existing plots
all_results = all_results_iters['iter=1000']
# endregion
...
# ── Plot 7: Iteration convergence comparison ──────────────────────────────────
_iter_colors = plt.cm.plasma(np.linspace(0.1, 0.9, len(_ITER_CONFIGS)))
fig7, axes7 = plt.subplots(2, 2, figsize=(14, 10))
fig7.suptitle(f'Iteration convergence  |  T_in = {T_C_PROF} °C  |  U_a = {U_a:.0f} W/(m³·K)',
              fontsize=12)
ax7X, ax7q, ax7T, ax7e = axes7[0, 0], axes7[0, 1], axes7[1, 0], axes7[1, 1]
for (cfg_name, _), col in zip(_ITER_CONFIGS, _iter_colors):
    us_vals, X_vals, q_vals, T_vals, e_vals = [], [], [], [], []
    for i_us, u_s in enumerate(U_S_LIST):
        e = all_results_iters[cfg_name].get((T_C_PROF, i_us))
        if e and e['res'] is not None:
            res = e['res']
            us_vals.append(u_s * 1e3)
            X_vals.append(float(res['X_CO2'][-1]) * 100)
            q_vals.append(float(res['q'][0]))
            T_vals.append(float(np.max(res['T'])) - 273.15)
            e_vals.append(float(res['conv_err']))
    if us_vals:
        ax7X.plot(us_vals, X_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
        ax7q.plot(us_vals, q_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
        ax7T.plot(us_vals, T_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
        ax7e.plot(us_vals, e_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
ax7X.set_xlabel('u_s [mm/s]', fontsize=11);  ax7X.set_ylabel('CO2 conversion [%]', fontsize=11)
ax7X.set_title('X_CO2 at exit', fontsize=10)
ax7X.legend(fontsize=8);  ax7X.grid(True, alpha=0.3)
ax7q.set_xlabel('u_s [mm/s]', fontsize=11);  ax7q.set_ylabel('q(z=0) [mol/kg]', fontsize=11)
ax7q.set_title('Solid H2O loading at solid exit', fontsize=10)
ax7q.legend(fontsize=8);  ax7q.grid(True, alpha=0.3)
ax7T.set_xlabel('u_s [mm/s]', fontsize=11);  ax7T.set_ylabel('T_max [°C]', fontsize=11)
ax7T.set_title('Peak temperature', fontsize=10)
ax7T.legend(fontsize=8);  ax7T.grid(True, alpha=0.3)
ax7e.set_xlabel('u_s [mm/s]', fontsize=11);  ax7e.set_ylabel('conv_err  [-]', fontsize=11)
ax7e.set_title('Residual convergence error at end of iterations', fontsize=10)
ax7e.axhline(1e-4, color='k', lw=1.5, ls='--', label='tol = 1e-4')
ax7e.set_yscale('log')
ax7e.legend(fontsize=8);  ax7e.grid(True, alpha=0.3, which='both')
plt.tight_layout()
_savefig('flux_plot7_iteration_convergence.png');  plt.show()

# ── Plot 8: H2O water balance check ──────────────────────────────────────────
# Balance: H2O produced (2·ΔF_CO2) = H2O out on gas (F_H2O at z=L)
#                                   + H2O out on solid (u_s·ρ_ads·q(z=0))
fig8, (ax8L, ax8R) = plt.subplots(1, 2, figsize=(14, 5))
fig8.suptitle(f'H2O water balance check  |  T_in = {T_C_PROF} °C  |  U_a = {U_a:.0f} W/(m³·K)',
              fontsize=12)

# Left panel: absolute fluxes for iter=1000 (baseline)
us_b, H2O_prod_b, H2O_gas_b, H2O_sol_b = [], [], [], []
for i_us, u_s in enumerate(U_S_LIST):
    e = all_results.get((T_C_PROF, i_us))
    if e and e['res'] is not None:
        res = e['res']
        H2O_prod = 2.0 * (F_in_CO2 - res['F_CO2_out'])
        H2O_gas  = res['F_H2O_out']
        H2O_sol  = u_s * rho_bed_ads * float(res['q'][0])
        us_b.append(u_s * 1e3)
        H2O_prod_b.append(H2O_prod)
        H2O_gas_b.append(H2O_gas)
        H2O_sol_b.append(H2O_sol)
H2O_sum_b = [g + s for g, s in zip(H2O_gas_b, H2O_sol_b)]
ax8L.plot(us_b, H2O_prod_b, 'k-o',  lw=2, ms=5, label='H2O produced  (2·ΔF_CO2)')
ax8L.plot(us_b, H2O_gas_b,  '-o',   lw=2, ms=5, color='tab:blue',   label='H2O out gas   (F_H2O at z=L)')
ax8L.plot(us_b, H2O_sol_b,  '-o',   lw=2, ms=5, color='tab:orange', label='H2O out solid (u_s·ρ_ads·q(z=0))')
ax8L.plot(us_b, H2O_sum_b,  '--^',  lw=2, ms=5, color='tab:green',  label='gas + solid  (should = produced)')
ax8L.set_xlabel('u_s [mm/s]', fontsize=11)
ax8L.set_ylabel('H2O molar flux  [mol/(m²·s)]', fontsize=11)
ax8L.set_title('iter=1000  |  absolute H2O fluxes', fontsize=10)
ax8L.legend(fontsize=8);  ax8L.grid(True, alpha=0.3)

# Right panel: relative balance (%) per iter config — should be 100 %
for (cfg_name, _), col in zip(_ITER_CONFIGS, _iter_colors):
    us_vals, bal_vals = [], []
    for i_us, u_s in enumerate(U_S_LIST):
        e = all_results_iters[cfg_name].get((T_C_PROF, i_us))
        if e and e['res'] is not None:
            ...
            if H2O_prod > 1e-20:
                us_vals.append(u_s * 1e3)
                bal_vals.append((H2O_gas + H2O_sol) / H2O_prod * 100)
    if us_vals:
        ax8R.plot(us_vals, bal_vals, marker='o', lw=2, ms=5, color=col, label=cfg_name)
ax8R.axhline(100.0, color='k', lw=1.5, ls='--', label='perfect balance (100 %)')
ax8R.set_xlabel('u_s [mm/s]', fontsize=11)
ax8R.set_ylabel('(H2O_gas + H2O_solid) / H2O_produced  [%]', fontsize=11)
ax8R.set_title('relative H2O balance  (should be ≈ 100 %)', fontsize=10)
ax8R.legend(fontsize=8);  ax8R.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('flux_plot8_H2O_balance.png');  plt.show()
# endregion
