...
Solved by decoupled Gauss-Seidel iteration with Anderson mixing:
    ...
    3. Anderson mixing update using last m residuals instead of fixed 0.5 damping.
...
# --- MPB scan parameters (high → low for warm-start continuation) ---
U_S_LIST  = np.array([6.0, 5.0, 4.0, 3.0, 2.75, 2.5, 2.25, 2.0, 1.75, 1.5, 1.25, 1.0, 0.75, 0.5]) * 1e-3
T_IN_LIST = [280]

print(f"MPB flux form (isothermal, Anderson mixing): d={d_b*100:.0f} cm, L={L_b:.1f} m, "
...
# region 3. DECOUPLED SOLVER WITH ANDERSON MIXING
# =============================================================================
def solve_mpb(u_s, T_K, max_iter=1000, tol=1e-5, N=200, q_init=None, m=5, beta=0.5):
    """
    Counter-current MPB — molar flux form, isothermal, Anderson mixing.
    ...
    Anderson mixing (depth m) replaces the fixed 0.5 damping step.
    Falls back to damped Picard for the first iteration (insufficient history).
    """
    ...
    # Anderson mixing history buffers
    hist_q = []   # q_prof snapshots
    hist_f = []   # residuals f = q_new - q_prof
    ...
    for it in range(max_iter):
        ...
        f = q_new - q_prof  # Picard residual

        hist_q.append(q_prof.copy())
        hist_f.append(f.copy())
        if len(hist_q) > m:
            hist_q.pop(0);  hist_f.pop(0)

        mk = len(hist_f)
        if mk < 2:
            # not enough history yet — damped Picard step
            q_prof_new = q_prof + beta * f
        else:
            # build difference matrices (N x mk-1)
            dF = np.column_stack([hist_f[i+1] - hist_f[i] for i in range(mk-1)])
            dX = np.column_stack([hist_q[i+1] - hist_q[i] for i in range(mk-1)])
            # find gamma minimising ||dF @ gamma - f||
            gamma, *_ = np.linalg.lstsq(dF, f, rcond=None)
            q_prof_new = q_prof + beta * f - (dX + beta * dF) @ gamma

        q_prof_new = np.maximum(q_prof_new, 0.0)  # q can't be negative

        scale = max(np.max(np.abs(q_new)), 1e-8)
        err   = np.max(np.abs(f)) / scale          # true fixed-point residual
        q_prof = q_prof_new
...
for T_C in T_IN_LIST:
    ...
    print(f"\n{'='*60}")
    print(f"  T_in = {T_C} C  |  u_g_STP = {u_g_STP*1e3:.1f} mm/s  |  isothermal (Anderson)")
    print(f"{'='*60}")
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        res = solve_mpb(u_s, T_K, q_init=q_init)
...
fig.suptitle(f'MPB flux form (isothermal, Anderson)  |  T = {T_C_PROF} C  |  counter-current', fontsize=11)
...
_qs_proxy = mlines.Line2D([], [], color='grey', lw=1.5, ls='--', alpha=0.6, label='q* (isotherm, dashed)')
...
plt.tight_layout()
_savefig(f'anderson_plot1_axial_profiles_T{T_C_PROF}C.png');  plt.show()
...
ax.set_title(f'MPB flux form (isothermal, Anderson)  — CO2 conversion vs solid velocity', fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('anderson_plot2_conversion_vs_us.png');  plt.show()
...
ax.set_title(f'MPB flux form (isothermal, Anderson)  — Sorbent utilisation', fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('anderson_plot3_sorbent_utilisation.png');  plt.show()

# ── Plot 4: H2O profiles ─────────────────────────────────────────────────────
T_K_prof = T_C_PROF + 273.15
fig, (ax_q, ax_h) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O profiles  |  T = {T_C_PROF} C  |  isothermal (Anderson)', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
...
plt.tight_layout()
_savefig(f'anderson_plot4_H2O_profiles_T{T_C_PROF}C.png');  plt.show()
# endregion
