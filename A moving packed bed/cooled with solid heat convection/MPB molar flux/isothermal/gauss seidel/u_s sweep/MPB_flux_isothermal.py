...
for T_C in T_IN_LIST:
    ...
    print(f"\n{'='*60}")
    print(f"  T_in = {T_C} C  |  u_g_STP = {u_g_STP*1e3:.1f} mm/s  |  isothermal")
    print(f"{'='*60}")
...
fig.suptitle(f'MPB flux form (isothermal)  |  T = {T_C_PROF} C  |  counter-current', fontsize=11)
...
plt.tight_layout()
_savefig(f'iso_plot1_axial_profiles_T{T_C_PROF}C.png');  plt.show()
...
plt.tight_layout()
_savefig('iso_plot2_conversion_vs_us.png');  plt.show()
...
ax.set_title(f'MPB flux form (isothermal)  — Sorbent utilisation', fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('iso_plot3_sorbent_utilisation.png');  plt.show()

# ── Plot 4: H2O profiles ─────────────────────────────────────────────────────
T_K_prof = T_C_PROF + 273.15
fig, (ax_q, ax_h) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O profiles  |  T = {T_C_PROF} C  |  isothermal', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        continue
...
plt.tight_layout()
_savefig(f'iso_plot4_H2O_profiles_T{T_C_PROF}C.png');  plt.show()
# endregion
