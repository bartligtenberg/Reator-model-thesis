...
# --- MPB scan parameters ---
U_S_LIST  = np.array([10.0, 8.0, 6.0, 5.0, 4.0, 3.0, 2.5, 2.0, 1.5, 1.0]) * 1e-3
T_IN_LIST = [280]
...
# =============================================================================
def solve_mpb(u_s, T_K, T_wall=None, max_iter=800, tol=1e-5, N=200, q_init=None):
    """
    ...
    # Under-relaxation: scale with u_s to damp oscillations in the stiff
    # solid-dominated regime where K_LDF/u_s can reach 20 000 m⁻¹.
    alpha = min(0.5, max(0.05, 0.15 * u_s / 1e-3))
...
def _q_physics_init(T_K, u_s_first, N=150):
    """
    Gas-only IVP (no adsorption) to get p_H2O(z), then solid LDF IVP from the
    solid inlet (zeta=0, z=L, q=0) to give a physically shaped initial q profile.
    Correct BC (q=0 at solid inlet) and correct peak location (near z=L).
    """
    def rhs_noads(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
        F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
    ...
    F_CO2_f = np.maximum(sol.y[0], 0.0)
    F_tot_f = np.maximum(sol.y[0] + sol.y[1] + sol.y[2] + sol.y[3], 1e-30)
    p_H2O_noads = np.maximum(sol.y[3], 0.0) / F_tot_f * P_bar
    p_H2O_fn = interp1d(z_grid, p_H2O_noads, kind='linear',
                        bounds_error=False,
                        fill_value=(p_H2O_noads[0], p_H2O_noads[-1]))

    # Solid IVP: integrate from zeta=0 (solid inlet, z=L, q=0) to zeta=L (z=0)
    def solid_init_rhs(zeta, q_arr):
        z_pos   = L_b - float(zeta)
        p_H2O_l = max(float(p_H2O_fn(z_pos)), 0.0)
        qs  = float(q_star(T_K, np.array([p_H2O_l]))[0])
        Kl  = float(K_LDF(T_K,  np.array([p_H2O_l]))[0])
        return [Kl * (qs - max(float(q_arr[0]), 0.0)) / u_s_first]

    ss = solve_ivp(solid_init_rhs, [0.0, L_b], [0.0],
                   method='BDF', rtol=1e-4, atol=1e-8,
                   t_eval=np.linspace(0.0, L_b, N))
    z_from_zeta = L_b - ss.t
    q_from_zeta = np.maximum(ss.y[0], 0.0)
    sort_idx    = np.argsort(z_from_zeta)
    q_profile   = np.interp(z_grid, z_from_zeta[sort_idx], q_from_zeta[sort_idx])

    return dict(q=q_profile,
...
for T_C in T_IN_LIST:
    ...
    _phys  = _q_physics_init(T_K, U_S_LIST[0])
    q_init = _phys['q']
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        src = "physics_init" if i_us == 0 else f"prev u_s={U_S_LIST[i_us-1]*1e3:.1f} mm/s"
        print(f"    init q [{src}]:  q(z=0)={q_init[0]:.4f}  q(z=L)={q_init[-1]:.4f}  mean={np.mean(q_init):.4f} mol/kg")
        init_q_store.append((u_s, q_init.copy(), src))
...
# ── Plot 7: Initial q guesses ─────────────────────────────────────────────────
z_init = np.linspace(0, L_b, 150)
fig, ax_ig = plt.subplots(figsize=(9, 5))
for k, (u_s_ig, q_ig, src_ig) in enumerate(init_q_store):
    ax_ig.plot(z_init, q_ig, color=pal[k], lw=2,
               label=f"u_s={u_s_ig*1e3:.1f} mm/s  [{src_ig}]")
ax_ig.set_xlabel('z [m]', fontsize=11)
ax_ig.set_ylabel('q_init [mol/kg]', fontsize=11)
ax_ig.set_title(f'Initial q guess per u_s  |  T_in = {T_C_PROF} C', fontsize=11)
ax_ig.legend(fontsize=7)
ax_ig.grid(True, alpha=0.3)
plt.tight_layout()
_savefig(f'flux_plot7_init_guess_T{T_C_PROF}C.png');  plt.show()
# endregion
