...
MOLAR FLUX FORM  —  gradient-ads convergence fix
==========================================================================================
...
CONVERGENCE FIX (gradient-ads mode, u_s < GRAD_ADS_THRESHOLD):
    The standard Gauss-Seidel loop uses ads = K_LDF*(q_star - q_fn) in the gas ODE,
    where q_fn is the LAGGED q profile from the previous iteration.  At low u_s the
    q gradient is steep; a small error Δq is amplified by K_LDF·ρ_bed_ads into a
    large error in dF_H2O/dz, causing oscillation and divergence.

    Fix: replace the lagged K_LDF*(q_star - q_fn) term in the GAS ODEs with
         ads_grad = u_s·(−dq_prof/dz)
    which follows from the solid mass balance (exact identity at convergence).
    Error during transients is reduced by factor 1/St = u_s/(K_LDF·L_b).

    The SOLID ODEs are unchanged — they still use K_LDF·(q_star − q)/u_s with the
    correct counter-current BC q(ζ=0) = 0, so q_out > 0 is preserved.
"""
...
print(f"MPB flux form (gradient-ads fix): d={d_b*100:.0f} cm, L={L_b:.1f} m, "
...
# =============================================================================
# Gradient-ads threshold: below this u_s, use ads = u_s*(−dq/dz) in the gas
# ODEs instead of the lagged K_LDF*(q_star − q_fn).  The solid ODEs are
# unchanged.  See module docstring for the physical justification.
GRAD_ADS_THRESHOLD = 3e-3   # [m/s]
...
def solve_mpb(u_s, T_K, T_wall=None, max_iter=600, tol=1e-5, N=400, q_init=None):
    ...
    # ── Gradient-ads mode and damping ────────────────────────────────────────
    use_grad_ads = (u_s < GRAD_ADS_THRESHOLD)
    omega = 0.3 if use_grad_ads else 0.5   # under-relaxation factor
    ...
    for it in range(max_iter):
        ...
        # ── Pre-compute gradient-based ads profile from current q_prof ────────
        # ads = u_s·(−dq/dz)  [mol/(kg_ads·s)]  from solid mass balance identity.
        # At convergence this equals K_LDF*(q_star − q) exactly.
        # During transients the error is reduced by 1/St vs the lagged LDF form.
        if use_grad_ads:
            dq_dz         = np.gradient(q_prof, z_grid)
            ads_grad_prof = u_s * (-dq_dz)
            ads_grad_fn   = interp1d(z_grid, ads_grad_prof, kind='linear',
                                     bounds_error=False,
                                     fill_value=(ads_grad_prof[0], ads_grad_prof[-1]))
        ...
        if gas_dominates:
            ...
            def gas_rhs(z, y):
                ...
                # gradient-ads: replaces Kl*(qs-q_l) to avoid K_LDF amplification
                ads = float(ads_grad_fn(z)) if use_grad_ads else Kl*(qs - q_l)
            ...
            # ── SOLID IVP: state = [q]  (unchanged — LDF kinetics, q(ζ=0)=0) ──
            ...
            # ── SOLID IVP: state = [q, T]  (unchanged — LDF kinetics, q(ζ=0)=0) ──
...
for T_C in T_IN_LIST:
    ...
    print(f"  u_s* = {u_s_star*1e3:.3f} mm/s  (regime switch)  |  grad-ads below {GRAD_ADS_THRESHOLD*1e3:.1f} mm/s")
    print(f"{'='*60}")
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        if res is not None:
            ...
            grad   = "(grad-ads)" if u_s < GRAD_ADS_THRESHOLD else ""
            print(f"  u_s={u_s*1e3:.4f} mm/s  X={X_out:.1f}%  "
                  f"q(0)={q_out:.3f}  T_max={T_max:.1f} C  "
                  f"[{regime}-dom, {tag}, {res['n_iter']} iter, err={res['conv_err']:.2e}] {grad}"
...
for T_C in T_IN_LIST:
    ...
    for i_us in reversed(range(len(U_S_LIST))):
        ...
        if res_new is not None:
            ...
            if improved:
                all_results[(T_C, i_us)]['res'] = res_new
            print(f"  RETRY u_s={u_s*1e3:.2f} mm/s  [{tag}, {res_new['n_iter']} iter, "
            ...
            print(f"  RETRY u_s={u_s*1e3:.2f} mm/s  FAILED  ({dt:.1f}s)")
...
fig.suptitle(f'MPB flux form (grad-ads)  |  T_in = {T_C_PROF} C  |  '
...
ax.set_title(f'MPB flux form (grad-ads)  |  U_a = {U_a:.0f} W/(m³·K)  — CO2 conversion vs solid velocity',
             fontsize=10)
...
ax.set_title(f'MPB flux form (grad-ads)  |  U_a = {U_a:.0f} W/(m³·K)  — Sorbent utilisation', fontsize=10)
...
fig.suptitle(f'H2O profiles (grad-ads)  |  T_in = {T_C_PROF} C  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
        continue
...
fig.suptitle(f'Temperature profile (grad-ads)  |  T_in = {T_C_PROF} C  |  U_a = {U_a:.0f} W/(m³·K)',
             fontsize=11)
for k, i_us in enumerate(i_plot):
    e = all_results.get((T_C_PROF, i_us))
    if e is None or e['res'] is None:
...
ax6.set_title(f'MPB flux form (grad-ads)  |  U_a = {U_a:.0f} W/(m³·K)  — peak temperature rise',
              fontsize=10)
...
fig.suptitle(f'H2O budget (grad-ads)  |  T_in = {T_C_PROF} C  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=11)
