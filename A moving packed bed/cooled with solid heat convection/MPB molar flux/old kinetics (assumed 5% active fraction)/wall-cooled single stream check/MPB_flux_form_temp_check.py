...
# --- LHHW kinetics (Wei et al., K_mix corrected to bar^-0.5) --- is this really wei
T_ref_K = 555.0
k_ref   = 3.46e-4
Ea_k    = 77.5e3
...
# --- MPB scan parameters ---
U_S_LIST  = np.array([4.0, 5.0, 6.0]) * 1e-3
T_IN_LIST = [280]
RUN_VALIDATION = True
...
# =============================================================================
def solve_mpb(u_s, T_K, T_wall=None, max_iter=600, tol=1e-5, N=400, q_init=None, sources_off=False):
    """
...
def validate_wall_cooling_analytical(T_in_K=553.15, T_wall_K=503.15, N=400):
    """
    Checks the energy-balance ODE integrator against the exact solution for
    wall-cooled gas with no reaction, no adsorption, and u_s = 0.

    With all sources off, the energy balance reduces to:
        gas_cap_in · dT/dz = -U_a · (T - T_wall)

    Exact solution (gas_cap_in constant when r = 0):
        T(z) = T_wall + (T_in - T_wall) · exp(-U_a · z / gas_cap_in)

    T_wall must differ from T_in to produce a non-trivial exponential profile.
    """
    gas_cap_in = _gas_cap(F_in_CO2, F_in_H2, F_in_CH4, 0.0)  # Σ F_i·Cp_i at inlet [W/(m²·K)]; constant because no reaction

    def rhs(_z, y):
        return [-(U_a * (y[0] - T_wall_K)) / gas_cap_in]  # dT/dz = -U_a·(T - T_wall) / gas_cap_in

    def analytical(z_arr):
        return T_wall_K + (T_in_K - T_wall_K) * np.exp(-U_a * z_arr / gas_cap_in)  # exact exponential decay toward T_wall

    print(f"\n{'='*60}")
    print(f"  Analytical validation: wall-cooled gas, no chemistry")
    print(f"  T_in = {T_in_K - 273.15:.1f} °C  |  T_wall = {T_wall_K - 273.15:.1f} °C")
    print(f"  gas_cap_in = {gas_cap_in:.5f} W/(m²·K)  |  U_a = {U_a:.0f} W/(m³·K)")
    print(f"  characteristic length = {gas_cap_in/U_a:.4f} m  (gas_cap_in / U_a)")
    print(f"{'='*60}")
    print(f"  {'N':>6}   max|err| [K]   status")
    for Ni in [50, 100, 200, 400, 800]:
        z_g = np.linspace(0.0, L_b, Ni)
        sol = solve_ivp(rhs, [0.0, L_b], [T_in_K],
                        method='BDF', rtol=1e-8, atol=1e-10,
                        t_eval=z_g, dense_output=False)
        err = np.max(np.abs(sol.y[0] - analytical(z_g)))
        print(f"  {Ni:>6}   {err:.3e}          {'PASS' if err < 1e-3 else 'FAIL'}")
    ...
    sol        = solve_ivp(rhs, [0.0, L_b], [T_in_K],
                           ...
                           t_eval=z_grid, dense_output=False)
    T_numeric  = sol.y[0]
    T_analytic = analytical(z_grid)
    max_abs_err = np.max(np.abs(T_numeric - T_analytic))
    print(f"\n  N={N} (plot): max |T_num - T_analytic| = {max_abs_err:.3e} K  "
          f"[{'PASS' if max_abs_err < 1e-3 else 'FAIL'}]")

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle(
        f"Analytical validation — wall-cooled gas, no chemistry\n"
        f"T_in = {T_in_K-273.15:.0f} °C  |  T_wall = {T_wall_K-273.15:.0f} °C  |  "
        f"U_a = {U_a:.0f} W/(m³·K)",
        fontsize=10)
    ax1.plot(z_grid, T_numeric - 273.15, lw=2,
             label='Numerical (BDF)', color='tab:blue')
    step = max(1, N // 40)
    ax1.plot(z_grid[::step], (T_analytic - 273.15)[::step],
             'o', ms=5, label='Analytical', color='tab:orange', zorder=5)
    ax1.axhline(T_wall_K - 273.15, color='grey', ls='--', lw=1.2,
                label=f'T_wall = {T_wall_K-273.15:.0f} °C')
    ax1.set_xlabel('z [m]');  ax1.set_ylabel('T [°C]')
    ax1.set_title('Temperature profile')
    ax1.legend(fontsize=9);  ax1.grid(True, alpha=0.3)
    ax2.semilogy(z_grid, np.abs(T_numeric - T_analytic), lw=2, color='tab:red')
    ax2.set_xlabel('z [m]');  ax2.set_ylabel('|T_num − T_analytic| [K]')
    ax2.set_title('Absolute error vs z')
    ax2.grid(True, alpha=0.3)
    plt.tight_layout()
    out_path = os.path.join(SAVE_DIR, 'validation_wall_cooling.png')
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"  Figure saved: {out_path}")
    plt.show()


def validate_countercurrent_analytical(T_in_K=553.15, T_wall_K=503.15, N=400):
    """
    Checks solve_mpb (sources_off=True) against the exact exponential solution
    for a counter-current bed with wall cooling but no reaction or adsorption.

    Exact solution (C_net constant when sources_off):
        C_net · dT/dz = -U_a·(T - T_wall),   C_net = gas_cap_in - u_s·rho_bed_tot·Cp_cat
        T(z) = T_wall + (T_in - T_wall)·exp(-U_a·z / C_net)

    Sweeps u_s values straddling u_s* (the regime-switch singularity).
    """
    gas_cap_in = _gas_cap(F_in_CO2, F_in_H2, F_in_CH4, 0.0)  # Σ F_i·Cp_i at inlet [W/(m²·K)]
    u_s_star   = gas_cap_in / (rho_bed_tot * Cp_cat)           # regime-switch velocity [m/s]
    u_s_list   = u_s_star * np.array([0.5, 0.8, 0.95, 1.05, 1.2, 1.5, 2.0, 4.0, 8.0, 10.75])

    tol = 1e-2
    print(f"\n{'='*60}")
    print(f"  Analytical validation: counter-current, sources off")
    print(f"  T_in = {T_in_K - 273.15:.1f} °C  |  T_wall = {T_wall_K - 273.15:.1f} °C")
    print(f"  u_s* = {u_s_star*1e3:.4f} mm/s  (gas_cap_in / rho_bed_tot·Cp_cat)")
    print(f"{'='*60}")
    print(f"  {'u_s [mm/s]':>12}  {'u_s/u_s*':>9}  {'regime':>6}  {'max_err [K]':>12}  status")

    results = []
    pal_v = plt.cm.plasma(np.linspace(0.1, 0.85, len(u_s_list)))

    fig, (ax_T, ax_e) = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(
        f'Analytical validation — counter-current, sources off\n'
        f'T_in={T_in_K-273.15:.0f} °C, T_wall={T_wall_K-273.15:.0f} °C', fontsize=11)

    for k, u_s in enumerate(u_s_list):
        res = solve_mpb(u_s, T_in_K, T_wall=T_wall_K, N=N, sources_off=True)
        if res is None:
            print(f"  {'---':>12}  {'---':>9}  {'---':>6}  {'SOLVER FAIL':>12}")
            continue

        z          = res['z']
        T_numeric  = res['T']
        C_net = gas_cap_in - u_s * rho_bed_tot * Cp_cat
        if res['gas_dominates']:
            # BC at z=0 (gas inlet): T decays from T_in toward T_wall along z
            T_analytic = T_wall_K + (T_in_K - T_wall_K) * np.exp(-U_a * z / C_net)
        else:
            # BC at z=L (solid inlet): T decays from T_in toward T_wall moving toward z=0
            T_analytic = T_wall_K + (T_in_K - T_wall_K) * np.exp(-U_a * (L_b - z) / (-C_net))
        max_err    = np.max(np.abs(T_numeric - T_analytic))
        regime     = 'gas' if res['gas_dominates'] else 'solid'
        status     = 'PASS' if max_err < tol else 'FAIL'
        ratio      = u_s / u_s_star

        print(f"  {u_s*1e3:>12.4f}  {ratio:>9.3f}  {regime:>6}  {max_err:>12.3e}  {status}")
        results.append((u_s, ratio, regime, max_err))

        step = max(1, len(z) // 40)
        lbl  = f"u_s={u_s*1e3:.3f} mm/s ({regime})"
        ax_T.plot(z, T_numeric - 273.15,                    color=pal_v[k], lw=2,   label=lbl)
        ax_T.plot(z[::step], (T_analytic - 273.15)[::step], color=pal_v[k], marker='o', ms=4,
                  ls='none', zorder=5)

    if results:
        ratios   = [r[1] for r in results]
        max_errs = [r[3] for r in results]
        ax_e.semilogy(ratios, max_errs, 'o-', lw=2, color='tab:red', ms=6)
        ax_e.axhline(tol, color='grey', lw=1.5, ls='--', label=f'tol = {tol:.0e} K')
        ax_e.set_xlabel('u_s / u_s*');  ax_e.set_ylabel('max |T_num − T_analytic| [K]')
        ax_e.set_title('Error vs regime ratio');  ax_e.legend(fontsize=9);  ax_e.grid(True, alpha=0.3)

    ax_T.set_xlabel('z [m]');  ax_T.set_ylabel('T [°C]')
    ax_T.set_title('Numeric (lines) vs Analytical (markers)');  ax_T.legend(fontsize=8);  ax_T.grid(True, alpha=0.3)
    plt.tight_layout()
    out_path = os.path.join(SAVE_DIR, 'validation_countercurrent.png')
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"  Figure saved: {out_path}")
    plt.show()
...
if RUN_VALIDATION:
    validate_wall_cooling_analytical(T_in_K=280+273.15, T_wall_K=230+273.15)
    validate_countercurrent_analytical(T_in_K=280+273.15, T_wall_K=230+273.15)
