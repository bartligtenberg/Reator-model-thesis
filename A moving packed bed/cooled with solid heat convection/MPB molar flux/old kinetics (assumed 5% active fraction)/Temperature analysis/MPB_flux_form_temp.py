...
# --- MPB scan parameters ---
# Temperature sweep at fixed solid velocity (this file's purpose): u_s = 5 mm/s, T_in = 200-340 C in 20 C steps.
U_S_LIST  = np.array([5.0]) * 1e-3
T_IN_LIST = list(range(200, 301, 20))
...
all_results  = {}                                                   # keyed by (T_C, i_us): stores {'res':..., 'u_s':..., 'T_K':..., 'T_wall':...} for every (T_in, u_s) combination scanned
noSE_results = {}                                                   # keyed by T_C: stores _compute_noSE's output (X_CO2_noSE + profile), one per inlet temperature -- the fixed-bed reference lines used in the plots
n_total      = len(T_IN_LIST) * len(U_S_LIST)                       # total number of (T_in, u_s) combinations to solve, for the progress/ETA printout below
...
for T_C in T_IN_LIST:                                               # outer sweep: one pass per inlet temperature in T_IN_LIST (200-340 C in 20 C steps)
    ...
    for i_us, u_s in enumerate(U_S_LIST):                             # inner sweep: one solve_mpb call per solid velocity in U_S_LIST (single value, 5 mm/s), for this T_C
        t0  = time.perf_counter()                                     # start timer for this single solve, to report per-case runtime below
        res = solve_mpb(u_s, T_K, T_wall=T_wall, q_init=q_init)        # q_init is the _q_physics_init guess for this T_C (each T_C starts fresh; no warm-start across T_C since the isotherm/rates shift with T)
        dt  = time.perf_counter() - t0                                 # elapsed time for just this solve_mpb call
        n_done += 1                                                    # tally toward n_total, for the ETA estimate
        elapsed = time.perf_counter() - t_run_start                     # total elapsed time since the whole T_C/u_s scan started
...
# region 6. PLOTS
# =============================================================================
# This file's sweep variable is T_in (200-340 C); u_s is fixed at U_S_LIST[0] = 5 mm/s.
# Convention throughout: solid line = converged, dashed line = not converged.
...
markers  = ['o', 's', '^', 'D', 'v', 'P', '*', 'X']
i_us     = 0                       # only one u_s in U_S_LIST
u_s_fix  = U_S_LIST[i_us]
pal      = plt.cm.plasma(np.linspace(0.1, 0.85, len(T_IN_LIST)))

def _ls(res):
    return '-' if res['converged'] else '--'

def _lbl(T_C, res):
    return f"T_in = {T_C} C" + ("" if res['converged'] else " (nc)")

# ── Plot 1: Axial profiles — all T_in combined ───────────────────────────────
fig, axes = plt.subplots(2, 2, figsize=(13, 9))
fig.suptitle(f'MPB flux form  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  '
             f'U_a = {U_a:.0f} W/(m³·K), counter-current\n'
             f'solid = converged  |  dashed = not converged', fontsize=11)
for k, T_C in enumerate(T_IN_LIST):
    e = all_results.get((T_C, i_us))
    if e is None or e['res'] is None:
        continue
    r, c = e['res'], pal[k]
    lbl  = _lbl(T_C, r)
    axes[0,0].plot(r['z'], r['C_CO2']*1e3, color=c, lw=2, ls=_ls(r), label=lbl)
    axes[0,1].plot(r['z'], r['q'],          color=c, lw=2, ls=_ls(r), label=lbl)
    axes[1,0].plot(r['z'], r['X_CO2']*100,  color=c, lw=2, ls=_ls(r), label=lbl)
    axes[1,1].plot(r['z'], r['r']*1e3,      color=c, lw=2, ls=_ls(r), label=lbl)
...
plt.tight_layout()
_savefig('flux_plot1_axial_profiles_Tscan.png');  plt.show()

# ── Plot 2: CO2 conversion vs T_in ───────────────────────────────────────────
fig, ax = plt.subplots(figsize=(9, 5))
T_conv, X_conv, T_nc, X_nc = [], [], [], []
noSE_T, noSE_X, eq_T, eq_X = [], [], [], []
for T_C in T_IN_LIST:
    ...
    if e and e['res']:
        ...
        if m:
            if e['res']['converged']:
                T_conv.append(T_C);  X_conv.append(m['X_CO2']*100)
            else:
                T_nc.append(T_C);    X_nc.append(m['X_CO2']*100)
    if T_C in noSE_results:
        noSE_T.append(T_C);  noSE_X.append(noSE_results[T_C]['X_CO2_noSE']*100)
    eq_T.append(T_C);  eq_X.append(equilibrium_conversion(T_C+273.15))
if T_conv:
    ax.plot(T_conv, X_conv, marker='o', color='tab:blue', lw=2, ms=6, label='MPB (u_s = 5 mm/s)')
if T_nc:
    ax.plot(T_nc, X_nc, marker='o', color='tab:blue', lw=2, ms=6,
            ls='--', mfc='none', mew=1.5, label='MPB (not converged)')
if noSE_T:
    ax.plot(noSE_T, noSE_X, marker='x', color='steelblue', lw=1.5, ms=6, ls=':',
            label='Fixed bed (no SE)')
ax.plot(eq_T, eq_X, color='k', lw=1, ls=':', alpha=0.6, label='Thermo. equilibrium')
ax.set_xlabel('T_in [°C]', fontsize=11);  ax.set_ylabel('CO2 conversion [%]', fontsize=11)
ax.set_title(f'MPB flux form  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  '
             f'— CO2 conversion vs T_in\nsolid = converged  |  dashed + open = not converged',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('flux_plot2_conversion_vs_Tin.png');  plt.show()

# ── Plot 3: Sorbent utilisation vs T_in ──────────────────────────────────────
fig, ax = plt.subplots(figsize=(9, 5))
T_conv, util_conv, T_nc, util_nc = [], [], [], []
for T_C in T_IN_LIST:
    e = all_results.get((T_C, i_us))
    if e and e['res']:
        m = get_metrics(e)
        if m:
            if e['res']['converged']:
                T_conv.append(T_C);  util_conv.append(m['sorbent_util']*100)
            else:
                T_nc.append(T_C);    util_nc.append(m['sorbent_util']*100)
if T_conv:
    ax.plot(T_conv, util_conv, marker='o', color='tab:blue', lw=2, ms=6, label='MPB (u_s = 5 mm/s)')
if T_nc:
    ax.plot(T_nc, util_nc, marker='o', color='tab:blue', lw=2, ms=6,
            ls='--', mfc='none', mew=1.5, label='MPB (not converged)')
ax.axhline(100, color='grey', lw=1.5, ls='--', label='q = q* (fully saturated)')
ax.set_xlabel('T_in [°C]', fontsize=11)
ax.set_ylabel('Sorbent utilisation  q(z=0) / q*(p_H2O_max)  [%]', fontsize=11)
ax.set_title(f'MPB flux form  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  '
             f'— Sorbent utilisation\nsolid = converged  |  dashed + open = not converged',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('flux_plot3_sorbent_utilisation_vs_Tin.png');  plt.show()

# ── Plot 4: H2O profiles — all T_in combined ─────────────────────────────────
fig, (ax_q, ax_h) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O profiles  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)\n'
             f'solid = converged  |  dashed = not converged', fontsize=11)
for k, T_C in enumerate(T_IN_LIST):
    e = all_results.get((T_C, i_us))
    if e is None or e['res'] is None:
    ...
    T_K_val = T_C + 273.15
    lbl     = _lbl(T_C, r)
    ax_q.plot(r['z'], r['q'],                        color=c, lw=2, ls=_ls(r), label=lbl)
    ax_h.plot(r['z'], r['C_H2O']*R_gas*T_K_val/1e2,  color=c, lw=2, ls=_ls(r), label=lbl)
...
plt.tight_layout()
_savefig('flux_plot4_H2O_profiles_Tscan.png');  plt.show()

# ── Plot 5: Temperature profiles — all T_in combined ─────────────────────────
fig, ax_T = plt.subplots(figsize=(9, 5))
fig.suptitle(f'Temperature profiles  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)\n'
             f'solid = converged  |  dashed = not converged', fontsize=11)
for k, T_C in enumerate(T_IN_LIST):
    e = all_results.get((T_C, i_us))
    if e is None or e['res'] is None:
    ...
    lbl    = f"T_in = {T_C} C ({regime})" + ("" if r['converged'] else " (nc)")
    ax_T.plot(r['z'], r['T'] - 273.15, color=pal[k], lw=2, ls=_ls(r), label=lbl)
...
plt.tight_layout()
_savefig('flux_plot5_temperature_Tscan.png');  plt.show()

# ── Plot 6: Peak temperature rise vs T_in ────────────────────────────────────
fig, ax6 = plt.subplots(figsize=(9, 5))
T_conv, dT_conv, T_nc, dT_nc = [], [], [], []
noSE_T6, noSE_dT = [], []
for T_C in T_IN_LIST:
    e = all_results.get((T_C, i_us))
    if e and e['res']:
        ...
        if m:
            if e['res']['converged']:
                T_conv.append(T_C);  dT_conv.append(m['T_max'] - (T_C + 273.15))
            else:
                T_nc.append(T_C);    dT_nc.append(m['T_max'] - (T_C + 273.15))
    if T_C in noSE_results and noSE_results[T_C].get('profile') is not None:
        noSE_T6.append(T_C)
        noSE_dT.append(float(np.max(noSE_results[T_C]['profile']['T'])) - (T_C + 273.15))
if T_conv:
    ax6.plot(T_conv, dT_conv, marker='o', color='tab:blue', lw=2, ms=6, label='MPB (u_s = 5 mm/s)')
if T_nc:
    ax6.plot(T_nc, dT_nc, marker='o', color='tab:blue', lw=2, ms=6,
             ls='--', mfc='none', mew=1.5, label='MPB (not converged)')
if noSE_T6:
    ax6.plot(noSE_T6, noSE_dT, marker='x', color='steelblue', lw=1.5, ms=6, ls=':',
             label='Fixed bed (no SE)')
ax6.set_xlabel('T_in [°C]', fontsize=11)
ax6.set_ylabel('ΔT_max = T_peak − T_in  [K]', fontsize=11)
ax6.set_title(f'MPB flux form  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  '
              f'— peak temperature rise\nsolid = converged  |  dashed + open = not converged',
              fontsize=10)
ax6.legend(fontsize=9);  ax6.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('flux_plot6_Tmax_vs_Tin.png');  plt.show()

# ── Plot 7: H2O budget decomposition — all T_in combined ─────────────────────
fig, (ax_rate, ax_cumul) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O budget  |  u_s = {u_s_fix*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)\n'
             f'solid = converged  |  dashed = not converged', fontsize=11)
...
for k, T_C in enumerate(T_IN_LIST):
    ...
    ax_rate.plot(r['z'], S_rxn*1e3, color=pal[k], lw=2,   ls=ls,            label=lbl)
    ax_rate.plot(r['z'], S_ads*1e3, color=pal[k], lw=1.5, ls=ls, alpha=0.5)

    ax_cumul.plot(r['z'], F_rxn*1e3, color=pal[k], lw=2,   ls=ls,            label=f"{lbl} rxn")
    ax_cumul.plot(r['z'], F_ads*1e3, color=pal[k], lw=1.5, ls=ls, alpha=0.5, label=f"{lbl} ads")
    ax_cumul.plot(r['z'], F_net*1e3, color=pal[k], lw=1,   ls=ls, alpha=0.3)
...
plt.tight_layout()
_savefig('flux_plot7_H2O_budget_Tscan.png');  plt.show()
# endregion
