"""
Moving Packed Bed (MPB) Reactor Model — Particle Diameter Sensitivity Scan
===========================================================================

Scans d_p = 0.75 → 5.0 mm to explore how particle size affects CO2 conversion.
Larger particles reduce K_LDF ∝ 1/d_p², which limits adsorption kinetics.

At high u_s (solid-dominated regime), reduced K_LDF can improve performance by
preventing H2O back-release at the gas inlet (z=0). This scan finds the crossover.

K_LDF ratio vs baseline (d_p = 0.75 mm):
    1.5 mm → 0.25×    3.0 mm → 0.06×    5.0 mm → 0.02×

All other parameters identical to MPB_flux_form.py (Bareschino setup, GHSV=0.5).
"""

import os
import time
import numpy as np
import matplotlib.pyplot as plt
from scipy.integrate import solve_ivp
from scipy.optimize import brentq
from scipy.interpolate import interp1d


# region 1. PARAMETERS
# =============================================================================

# --- Bed geometry (Bareschino lab setup) ---
d_b   = 0.050
L_b   = 2.000
A_b   = np.pi / 4 * d_b**2
V_bed = A_b * L_b
eps_b = 0.4

# --- Catalyst and sorbent loading ---
M_cat = 0.064
M_ads = 1.22
rho_bed_cat = M_cat / V_bed
rho_bed_ads = M_ads / V_bed
rho_bed_tot = (M_cat + M_ads) / V_bed

# --- Particle properties — d_p is updated in scan loop ---
D_P_LIST = [0.75e-3, 1.5e-3, 3.0e-3, 5.0e-3]
d_p   = D_P_LIST[0]   # active value; overwritten in scan loop below
eps_p = 0.242
tau_p = 3.0
rho_p = 1400   # [kg/m³]  particle density of sorbent (Bareschino)

# --- Dubinin-Astakhov isotherm (H2O on 13X) ---
W0_DA = 190.00e-6
E_DA  = 1190e3
n_DA  = 1.55

# --- LHHW kinetics (Wei et al., K_mix corrected to bar^-0.5) ---
T_ref_K = 555.0
k_ref   = 3.46e-4
Ea_k    = 77.5e3
A_OH    = 0.50;  dH_OH  =  22.4e3
A_H2    = 0.44;  dH_H2  =  -6.2e3
A_mix   = 0.88;  dH_mix = -10.0e3
P_FLOOR = 1e-4

# --- Thermochemistry ---
dH_r   = -165.0e3
dH_ads =  -45.0e3
Cp_cat = 1100.0
Cp_CO2 = 45.4;  Cp_H2 = 29.3;  Cp_CH4 = 46.9;  Cp_H2O = 34.2

# --- Wall heat transfer ---
U_a    = 2000.0

# --- Physical constants ---
R_gas  = 8.314
MW_H2O = 0.018015

# --- Operating conditions ---
P_bar = 1.0
P_Pa  = P_bar * 1e5
y_CO2_in = 0.04
y_H2_in  = 0.16
y_CH4_in = 0.80

# --- Inlet molar fluxes [mol/(m²·s)] — temperature-independent ---
T_STP   = 273.15
GHSV    = 0.5
Q_STP   = GHSV * M_ads / 3600.0
u_g_STP = Q_STP / A_b
F_total_in = u_g_STP * P_Pa / (R_gas * T_STP)
F_in_CO2   = y_CO2_in * F_total_in
F_in_H2    = y_H2_in  * F_total_in
F_in_CH4   = y_CH4_in * F_total_in

# --- Scan parameters ---
U_S_LIST  = np.array([1, 2.0, 2.5, 3.0, 4.0, 5, 6]) * 1e-3
T_IN_LIST = [280]

print(f"MPB d_p scan: d={d_b*100:.0f} cm, L={L_b:.1f} m, GHSV={GHSV}, U_a={U_a:.0f} W/(m3·K)")
print(f"  F_in_total = {F_total_in:.4f} mol/(m2·s),  u_g_STP = {u_g_STP*1e3:.1f} mm/s")
print(f"  d_p values: {[f'{v*1e3:.2g} mm' for v in D_P_LIST]}")
# endregion


# region 2. FUNCTIONS
# =============================================================================
def P_sat_bar(T_K):
    log10_p = (29.8605 - 3.1522e3/T_K - 7.3037*np.log10(T_K)
               + 2.4247e-9*T_K + 1.8090e-6*T_K**2)
    return 10.0**np.clip(log10_p, -10, 10) * 133.322e-5

def rho_water(T_K):
    return 996.0 / (1.0 + 2.0e-3*(T_K - 298.15))

def q_star_vec(T_K, p_arr, W0, E, n):
    p    = np.asarray(p_arr, dtype=float)
    Psat = P_sat_bar(T_K)
    p_safe = np.clip(p, 1e-15, Psat*(1-1e-10))
    A_raw  = (R_gas/MW_H2O)*T_K*np.log(Psat/p_safe)
    A  = np.where((p <= 0)|(p >= Psat), 0.0, A_raw)
    W  = W0*np.exp(-np.minimum((A/E)**n, 500.0))
    qs = rho_water(T_K)/MW_H2O*W
    return np.where(p <= 0, 0.0, qs)

def K_LDF_vec(T_K, p_arr, W0, E, n):
    D_M = 3.36e-9 * T_K**1.75
    p    = np.asarray(p_arr, dtype=float)
    dp   = 1.0/1e5
    dqsp = (q_star_vec(T_K, p+dp, W0, E, n)
            - q_star_vec(T_K, np.maximum(p-dp, 1e-15), W0, E, n)) / 2.0
    dqsp = np.maximum(dqsp, 1e-30)
    r_p  = 0.5 * d_p   # reads module-level d_p — updated by scan loop
    return 15.0 * eps_p * D_M / (r_p**2 * tau_p * rho_p * R_gas * T_K * dqsp)

def K_eq_sabatier(T_K):
    return 137.0*T_K**(-3.994)*np.exp(158700.0/(R_gas*T_K))

def reaction_rate_SI(T_K, p_CO2, p_H2, p_CH4, p_H2O):
    vH    = lambda dH: np.exp(-dH/R_gas*(1.0/T_K - 1.0/T_ref_K))
    k     = k_ref*np.exp(-Ea_k/R_gas*(1.0/T_K - 1.0/T_ref_K))
    K_OH  = A_OH*vH(dH_OH);  K_H2 = A_H2*vH(dH_H2);  K_mix = A_mix*vH(dH_mix)
    K_eq  = K_eq_sabatier(T_K)
    p_CO2_s = np.maximum(p_CO2, P_FLOOR);  p_H2_s = np.maximum(p_H2, P_FLOOR)
    beta  = (p_CH4*p_H2O**2)/(K_eq*p_CO2_s*p_H2_s**4)
    f_eq  = np.maximum(1.0 - np.where(np.isfinite(beta), beta, 1e10), 0.0)
    DEN   = (1.0 + K_OH*np.maximum(p_H2O, 0)/p_H2_s**0.5
             + K_H2*p_H2_s**0.5 + K_mix*p_CO2_s**0.5)
    return k*(p_CO2_s*p_H2_s)**0.5*f_eq/DEN**2*1000.0

def q_star(T_K, p_H2O):
    return q_star_vec(T_K, p_H2O, W0_DA, E_DA, n_DA)

_K_LDF_MAX = 20

def K_LDF(T_K, p_H2O):
    return np.minimum(K_LDF_vec(T_K, p_H2O, W0_DA, E_DA, n_DA), _K_LDF_MAX)

def equilibrium_conversion(T_K_val):
    K = K_eq_sabatier(T_K_val)
    def f(X):
        d = 1.0 - 0.08*X
        return ((0.80+0.04*X)/d * (0.08*X/d)**2
                / ((0.04*(1-X)/d)*(0.16*(1-X)/d)**4 + 1e-100) - K)
    try:
        return brentq(f, 1e-9, 1-1e-9)*100.0
    except Exception:
        return 100.0

def _gas_cap(F_CO2, F_H2, F_CH4, F_H2O):
    return F_CO2*Cp_CO2 + F_H2*Cp_H2 + F_CH4*Cp_CH4 + F_H2O*Cp_H2O

def _partial_pressures(F_CO2, F_H2, F_CH4, F_H2O):
    F_tot = F_CO2 + F_H2 + F_CH4 + F_H2O
    if F_tot < 1e-30:
        return 0.0, 0.0, 0.0, 0.0
    return (F_CO2/F_tot*P_bar, F_H2/F_tot*P_bar,
            F_CH4/F_tot*P_bar, F_H2O/F_tot*P_bar)
# endregion


# region 3. DECOUPLED SOLVER
# =============================================================================
def solve_mpb(u_s, T_K, T_wall=None, max_iter=600, tol=1e-4, N=100, q_init=None):
    """
    Counter-current MPB — molar flux form, lightly cooled, regime-switching.
    Uses current module-level d_p (via K_LDF_vec) — update d_p before calling.
    """
    if T_wall is None:
        T_wall = T_K

    solid_cap  = u_s * rho_bed_tot * Cp_cat
    gas_cap_in = _gas_cap(F_in_CO2, F_in_H2, F_in_CH4, 0.0)
    gas_dominates = (solid_cap < gas_cap_in)

    z_grid = np.linspace(0.0, L_b, N)
    if q_init is not None:
        q_prof = np.interp(z_grid, np.linspace(0, L_b, len(q_init)), q_init)
    else:
        q_prof = np.zeros(N)
    T_prof = T_K * np.ones(N)

    converged = False
    err       = 1.0
    omega     = 0.5
    err_prev  = np.inf

    for it in range(max_iter):
        q_fn = interp1d(z_grid, q_prof, kind='linear',
                        bounds_error=False, fill_value=(q_prof[0], q_prof[-1]))
        T_fn = interp1d(z_grid, T_prof, kind='linear',
                        bounds_error=False, fill_value=(T_prof[0], T_prof[-1]))

        if gas_dominates:
            # ── GAS IVP: state = [F_CO2, F_H2, F_CH4, F_H2O, T] ─────────────
            def gas_rhs(z, y):
                F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
                F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
                T_l     = max(y[4], 200.0)
                q_l     = max(float(q_fn(z)), 0.0)

                p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
                    F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)

                r   = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                              np.array([p_CH4]), np.array([p_H2O]))[0])
                qs  = float(q_star(T_l, np.array([p_H2O]))[0])
                Kl  = float(K_LDF(T_l,  np.array([p_H2O]))[0])
                ads = Kl*(qs - q_l)

                gas_cap_l = _gas_cap(F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
                denom     = gas_cap_l - solid_cap

                Q_rxn  = (-dH_r)   * rho_bed_cat * r
                Q_ads  = (-dH_ads) * rho_bed_ads * ads
                Q_wall = U_a * (T_l - T_wall)
                dTdz   = (Q_rxn + Q_ads - Q_wall) / denom

                return [
                    -rho_bed_cat * r,
                    -4.0*rho_bed_cat * r,
                    +rho_bed_cat * r,
                    2.0*rho_bed_cat*r - rho_bed_ads*ads,
                    dTdz,
                ]

            gs = solve_ivp(gas_rhs, [0.0, L_b],
                           [F_in_CO2, F_in_H2, F_in_CH4, 0.0, T_K],
                           method='BDF', rtol=1e-4,
                           atol=np.array([1e-8, 1e-8, 1e-8, 1e-8, 1e-2]),
                           t_eval=z_grid, dense_output=False)
            if not gs.success:
                return None

            F_CO2_prof = np.maximum(gs.y[0], 0.0);  F_H2_prof  = np.maximum(gs.y[1], 0.0)
            F_CH4_prof = np.maximum(gs.y[2], 0.0);  F_H2O_prof = np.maximum(gs.y[3], 0.0)
            T_prof_new = np.maximum(gs.y[4], 200.0)

            F_tot_prof  = np.maximum(F_CO2_prof + F_H2_prof + F_CH4_prof + F_H2O_prof, 1e-30)
            p_H2O_prof  = F_H2O_prof / F_tot_prof * P_bar
            p_H2O_fn    = interp1d(z_grid, p_H2O_prof, kind='linear',
                                   bounds_error=False,
                                   fill_value=(p_H2O_prof[0], p_H2O_prof[-1]))
            T_fn_new    = interp1d(z_grid, T_prof_new, kind='linear',
                                   bounds_error=False,
                                   fill_value=(T_prof_new[0], T_prof_new[-1]))

            # ── SOLID IVP: state = [q] ────────────────────────────────────────
            def solid_rhs(zeta, q_arr):
                z_pos    = L_b - float(zeta)
                T_local  = float(T_fn_new(z_pos))
                p_H2O_l  = max(float(p_H2O_fn(z_pos)), 0.0)
                qs  = float(q_star(T_local, np.array([p_H2O_l]))[0])
                Kl  = float(K_LDF(T_local,  np.array([p_H2O_l]))[0])
                q_val = max(float(q_arr[0]), 0.0)
                return [Kl*(qs - q_val)/u_s]

            ss = solve_ivp(solid_rhs, [0.0, L_b], [0.0],
                           method='BDF', rtol=1e-4, atol=1e-8,
                           t_eval=np.linspace(0.0, L_b, N), dense_output=False)
            if not ss.success:
                return None

            z_from_zeta = L_b - ss.t
            q_from_zeta = np.maximum(ss.y[0], 0.0)
            sort_idx    = np.argsort(z_from_zeta)
            q_new = np.interp(z_grid, z_from_zeta[sort_idx], q_from_zeta[sort_idx])

            scale = max(np.max(q_new), 1e-8)
            err   = np.max(np.abs(q_new - q_prof)) / scale

            if err > err_prev:
                omega = max(0.15, omega * 0.7)
            else:
                omega = min(0.60, omega * 1.05)
            err_prev = err

            q_prof_new = (1 - omega)*q_prof + omega*q_new
            T_prof     = (1 - omega)*T_prof + omega*T_prof_new
            q_prof = q_prof_new

        else:
            # ── GAS IVP: state = [F_CO2, F_H2, F_CH4, F_H2O]  (T frozen) ────
            def gas_rhs_no_T(z, y):
                F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
                F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
                T_l     = max(float(T_fn(z)), 200.0)
                q_l     = max(float(q_fn(z)), 0.0)

                p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
                    F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)

                r   = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                              np.array([p_CH4]), np.array([p_H2O]))[0])
                qs  = float(q_star(T_l, np.array([p_H2O]))[0])
                Kl  = float(K_LDF(T_l,  np.array([p_H2O]))[0])
                ads = Kl*(qs - q_l)

                return [
                    -rho_bed_cat * r,
                    -4.0*rho_bed_cat * r,
                    +rho_bed_cat * r,
                    2.0*rho_bed_cat*r - rho_bed_ads*ads,
                ]

            gs = solve_ivp(gas_rhs_no_T, [0.0, L_b],
                           [F_in_CO2, F_in_H2, F_in_CH4, 0.0],
                           method='BDF', rtol=1e-4,
                           atol=np.array([1e-8, 1e-8, 1e-8, 1e-8]),
                           t_eval=z_grid, dense_output=False)
            if not gs.success:
                return None

            F_CO2_prof = np.maximum(gs.y[0], 0.0);  F_H2_prof  = np.maximum(gs.y[1], 0.0)
            F_CH4_prof = np.maximum(gs.y[2], 0.0);  F_H2O_prof = np.maximum(gs.y[3], 0.0)
            F_tot_prof = np.maximum(F_CO2_prof + F_H2_prof + F_CH4_prof + F_H2O_prof, 1e-30)

            _make_fn = lambda p: interp1d(z_grid, p, kind='linear',
                                          bounds_error=False, fill_value=(p[0], p[-1]))
            F_CO2_fn = _make_fn(F_CO2_prof);  F_H2_fn  = _make_fn(F_H2_prof)
            F_CH4_fn = _make_fn(F_CH4_prof);  F_H2O_fn = _make_fn(F_H2O_prof)
            F_tot_fn = _make_fn(F_tot_prof)

            # ── SOLID IVP: state = [q, T] ─────────────────────────────────────
            def solid_rhs_with_T(zeta, y_arr):
                q_val = max(float(y_arr[0]), 0.0)
                T_val = max(float(y_arr[1]), 200.0)
                z_pos = L_b - float(zeta)

                F_CO2_l = max(float(F_CO2_fn(z_pos)), 0.0)
                F_H2_l  = max(float(F_H2_fn(z_pos)),  0.0)
                F_CH4_l = max(float(F_CH4_fn(z_pos)), 0.0)
                F_H2O_l = max(float(F_H2O_fn(z_pos)), 0.0)
                F_tot_l = max(float(F_tot_fn(z_pos)), 1e-30)

                p_CO2 = F_CO2_l/F_tot_l*P_bar;  p_H2  = F_H2_l /F_tot_l*P_bar
                p_CH4 = F_CH4_l/F_tot_l*P_bar;  p_H2O = F_H2O_l/F_tot_l*P_bar

                r   = float(reaction_rate_SI(T_val, np.array([p_CO2]), np.array([p_H2]),
                                              np.array([p_CH4]), np.array([p_H2O]))[0])
                qs  = float(q_star(T_val, np.array([p_H2O]))[0])
                Kl  = float(K_LDF(T_val, np.array([p_H2O]))[0])
                ads = Kl*(qs - q_val)

                gas_cap_l   = _gas_cap(F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
                solid_denom = solid_cap - gas_cap_l

                Q_rxn  = (-dH_r)   * rho_bed_cat * r
                Q_ads  = (-dH_ads) * rho_bed_ads * ads
                Q_wall = U_a * (T_val - T_wall)

                return [Kl*(qs - q_val)/u_s,
                        (Q_rxn + Q_ads - Q_wall) / solid_denom]

            ss = solve_ivp(solid_rhs_with_T, [0.0, L_b], [0.0, T_K],
                           method='BDF', rtol=1e-4, atol=np.array([1e-8, 1.0]),
                           max_step=1e-3,
                           t_eval=np.linspace(0.0, L_b, N), dense_output=False)
            if not ss.success:
                return None

            z_from_zeta  = L_b - ss.t
            q_from_zeta  = np.maximum(ss.y[0], 0.0)
            T_from_zeta  = np.maximum(ss.y[1], 200.0)
            sort_idx     = np.argsort(z_from_zeta)
            q_new  = np.interp(z_grid, z_from_zeta[sort_idx], q_from_zeta[sort_idx])
            T_new  = np.interp(z_grid, z_from_zeta[sort_idx], T_from_zeta[sort_idx])

            err_q = np.max(np.abs(q_new - q_prof)) / max(np.max(q_new), 1e-8)
            err_T = np.max(np.abs(T_new - T_prof)) / T_K
            err   = max(err_q, err_T)

            if err > err_prev:
                omega = max(0.15, omega * 0.7)
            else:
                omega = min(0.60, omega * 1.05)
            err_prev = err

            q_prof_new = (1 - omega)*q_prof + omega*q_new
            T_prof_new = (1 - omega)*T_prof + omega*T_new

            q_prof = q_prof_new
            T_prof = T_prof_new

        if err < tol:
            converged = True
            break

    # ── Final recompute on fine grid ─────────────────────────────────────────
    z_fine = np.linspace(0.0, L_b, 300)
    q_fn_f = interp1d(z_grid, q_prof, kind='linear',
                      bounds_error=False, fill_value=(q_prof[0], q_prof[-1]))
    T_fn_f = interp1d(z_grid, T_prof, kind='linear',
                      bounds_error=False, fill_value=(T_prof[0], T_prof[-1]))

    if gas_dominates:
        def gas_rhs_final(z, y):
            F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
            F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
            T_l     = max(y[4], 200.0)
            q_l     = max(float(q_fn_f(z)), 0.0)
            p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
                F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
            r   = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                          np.array([p_CH4]), np.array([p_H2O]))[0])
            qs  = float(q_star(T_l, np.array([p_H2O]))[0])
            Kl  = float(K_LDF(T_l, np.array([p_H2O]))[0])
            ads = Kl*(qs - q_l)
            gas_cap_l = _gas_cap(F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
            denom = gas_cap_l - solid_cap
            Q_rxn = (-dH_r)*rho_bed_cat*r;  Q_ads = (-dH_ads)*rho_bed_ads*ads
            Q_wall = U_a*(T_l - T_wall)
            return [
                -rho_bed_cat*r,
                -4.0*rho_bed_cat*r,
                +rho_bed_cat*r,
                2.0*rho_bed_cat*r - rho_bed_ads*ads,
                (Q_rxn + Q_ads - Q_wall)/denom,
            ]
        gf = solve_ivp(gas_rhs_final, [0.0, L_b],
                       [F_in_CO2, F_in_H2, F_in_CH4, 0.0, T_K],
                       method='BDF', rtol=1e-6,
                       atol=np.array([1e-10, 1e-10, 1e-10, 1e-10, 1e-3]),
                       t_eval=z_fine, dense_output=False)
        F_CO2f = np.maximum(gf.y[0], 0.0);  F_H2f  = np.maximum(gf.y[1], 0.0)
        F_CH4f = np.maximum(gf.y[2], 0.0);  F_H2Of = np.maximum(gf.y[3], 0.0)
        T_fine = np.maximum(gf.y[4], 200.0)

    else:
        def gas_rhs_final_no_T(z, y):
            F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
            F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
            T_l     = max(float(T_fn_f(z)), 200.0)
            q_l     = max(float(q_fn_f(z)), 0.0)
            p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
                F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
            r   = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                          np.array([p_CH4]), np.array([p_H2O]))[0])
            qs  = float(q_star(T_l, np.array([p_H2O]))[0])
            Kl  = float(K_LDF(T_l, np.array([p_H2O]))[0])
            ads = Kl*(qs - q_l)
            return [
                -rho_bed_cat*r,
                -4.0*rho_bed_cat*r,
                +rho_bed_cat*r,
                2.0*rho_bed_cat*r - rho_bed_ads*ads,
            ]
        gf = solve_ivp(gas_rhs_final_no_T, [0.0, L_b],
                       [F_in_CO2, F_in_H2, F_in_CH4, 0.0],
                       method='BDF', rtol=1e-6,
                       atol=np.array([1e-10, 1e-10, 1e-10, 1e-10]),
                       t_eval=z_fine, dense_output=False)
        F_CO2f = np.maximum(gf.y[0], 0.0);  F_H2f  = np.maximum(gf.y[1], 0.0)
        F_CH4f = np.maximum(gf.y[2], 0.0);  F_H2Of = np.maximum(gf.y[3], 0.0)
        T_fine = np.interp(z_fine, z_grid, T_prof)

    q_fine   = np.interp(z_fine, z_grid, q_prof)
    F_totf   = np.maximum(F_CO2f + F_H2f + F_CH4f + F_H2Of, 1e-30)
    p_CO2f   = F_CO2f/F_totf*P_bar;  p_H2f  = F_H2f /F_totf*P_bar
    p_CH4f   = F_CH4f/F_totf*P_bar;  p_H2Of = F_H2Of/F_totf*P_bar
    r_fine   = reaction_rate_SI(T_fine, p_CO2f, p_H2f, p_CH4f, p_H2Of)
    X_CO2    = np.clip(1.0 - F_CO2f/F_in_CO2, 0.0, 1.0)

    u_g_fine = F_totf * R_gas * T_fine / P_Pa
    C_CO2f   = F_CO2f / u_g_fine
    C_H2f    = F_H2f  / u_g_fine
    C_CH4f   = F_CH4f / u_g_fine
    C_H2Of   = F_H2Of / u_g_fine

    return dict(z=z_fine, C_CO2=C_CO2f, C_H2=C_H2f, C_CH4=C_CH4f,
                C_H2O=C_H2Of, q=q_fine, T=T_fine, r=r_fine, X_CO2=X_CO2,
                converged=converged, n_iter=it+1, conv_err=float(err),
                gas_dominates=gas_dominates)
# endregion


# region 4. SOLVE LOOP
# =============================================================================
def _compute_noSE(T_K, T_wall, N=300):
    def rhs(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
        F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
        T_l     = max(y[4], 200.0)
        p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
            F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
        r = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                   np.array([p_CH4]), np.array([p_H2O]))[0])
        gas_cap = _gas_cap(F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
        Q_rxn  = (-dH_r) * rho_bed_cat * r
        Q_wall = U_a * (T_l - T_wall)
        return [
            -rho_bed_cat*r, -4.0*rho_bed_cat*r,
            +rho_bed_cat*r,  2.0*rho_bed_cat*r,
            (Q_rxn - Q_wall) / gas_cap,
        ]
    z_grid = np.linspace(0, L_b, N)
    sol = solve_ivp(rhs, [0, L_b], [F_in_CO2, F_in_H2, F_in_CH4, 0.0, T_K],
                    method='BDF', rtol=1e-5,
                    atol=np.array([1e-9, 1e-9, 1e-9, 1e-9, 1e-2]), t_eval=z_grid)
    F_CO2_f = np.maximum(sol.y[0], 0.0)
    T_f     = np.maximum(sol.y[4], 200.0)
    F_tot_f = np.maximum(sol.y[0] + sol.y[1] + sol.y[2] + sol.y[3], 1e-30)
    p_H2O_f = np.maximum(sol.y[3], 0.0)/F_tot_f * P_bar
    p_CO2_f = F_CO2_f / F_tot_f * P_bar
    p_H2_f  = np.maximum(sol.y[1], 0.0)/F_tot_f * P_bar
    p_CH4_f = np.maximum(sol.y[2], 0.0)/F_tot_f * P_bar
    r_f     = reaction_rate_SI(T_f, p_CO2_f, p_H2_f, p_CH4_f, p_H2O_f)
    X_f     = np.clip(1.0 - F_CO2_f/F_in_CO2, 0.0, 1.0)
    u_g_f   = F_tot_f * R_gas * T_f / P_Pa
    return dict(X_CO2_noSE=float(X_f[-1]),
                profile=dict(z=z_grid, C_CO2=F_CO2_f/u_g_f,
                             C_H2O=np.maximum(sol.y[3], 0.0)/u_g_f,
                             q=q_star(T_f, p_H2O_f), r=r_f, X_CO2=X_f, T=T_f))

def _q_physics_init(T_K, N=150):
    def rhs_noads(_z, y):
        F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0)
        F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
        p_CO2, p_H2, p_CH4, p_H2O = _partial_pressures(
            F_CO2_l, F_H2_l, F_CH4_l, F_H2O_l)
        r = float(reaction_rate_SI(T_K, np.array([p_CO2]), np.array([p_H2]),
                                   np.array([p_CH4]), np.array([p_H2O]))[0])
        return [
            -rho_bed_cat*r, -4.0*rho_bed_cat*r,
            +rho_bed_cat*r,  2.0*rho_bed_cat*r,
        ]
    z_grid = np.linspace(0, L_b, N)
    sol = solve_ivp(rhs_noads, [0, L_b], [F_in_CO2, F_in_H2, F_in_CH4, 0.0],
                    method='BDF', rtol=1e-4, atol=1e-8, t_eval=z_grid)
    F_CO2_f = np.maximum(sol.y[0], 0.0)
    F_tot_f = np.maximum(sol.y[0] + sol.y[1] + sol.y[2] + sol.y[3], 1e-30)
    p_H2O_f = np.maximum(sol.y[3], 0.0)/F_tot_f * P_bar
    return dict(q=q_star(T_K, p_H2O_f))

def _fmt_seconds(s):
    s = int(s)
    return f"{s//60}m {s%60:02d}s" if s >= 60 else f"{s}s"

# --- Pre-compute temperature-independent quantities ---
T_C    = T_IN_LIST[0]
T_K    = T_C + 273.15
T_wall = T_K

gas_cap_in = _gas_cap(F_in_CO2, F_in_H2, F_in_CH4, 0.0)
u_s_star   = gas_cap_in / (rho_bed_tot * Cp_cat)

noSE = _compute_noSE(T_K, T_wall)
print(f"\n  T_in = {T_C} C  |  u_s* = {u_s_star*1e3:.3f} mm/s")
print(f"  non-SE fixed-bed conversion: {noSE['X_CO2_noSE']*100:.1f}%")

# --- Main scan ---
all_results_dp = {}   # {dp_label: {i_us: entry}}

n_total     = len(D_P_LIST) * len(U_S_LIST)
n_done      = 0
t_run_start = time.perf_counter()

for d_p_val in D_P_LIST:
    d_p   = d_p_val   # update module-level d_p used by K_LDF_vec
    label = f"{d_p_val*1e3:.2g} mm"
    all_results_dp[label] = {}

    kldf_ratio = (D_P_LIST[0] / d_p_val)**2
    print(f"\n{'='*65}")
    print(f"  d_p = {label}  (K_LDF × {kldf_ratio:.2f} vs baseline)")
    print(f"{'='*65}")

    q_init = _q_physics_init(T_K)['q'][::-1]

    for i_us, u_s in enumerate(U_S_LIST):
        t0  = time.perf_counter()
        res = solve_mpb(u_s, T_K, T_wall=T_wall, q_init=q_init,
                        max_iter=600, tol=1e-4, N=100)
        dt  = time.perf_counter() - t0
        n_done += 1
        elapsed = time.perf_counter() - t_run_start
        eta     = elapsed / n_done * (n_total - n_done)

        if res is not None:
            X_out  = float(res['X_CO2'][-1]) * 100
            q_out  = float(res['q'][0])
            T_max  = float(np.max(res['T'])) - 273.15
            regime = "gas" if res['gas_dominates'] else "solid"
            tag    = "ok" if res['converged'] else "not-conv"
            print(f"  u_s={u_s*1e3:.3f} mm/s  X={X_out:.1f}%  "
                  f"q(0)={q_out:.3f}  T_max={T_max:.1f} C  "
                  f"[{regime}-dom, {tag}, {res['n_iter']} it, err={res['conv_err']:.2e}]"
                  f"  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
            T_out      = float(res['T'][-1])
            y_CO2_out  = float(res['C_CO2'][-1]) * R_gas * T_out / P_Pa
            F_CO2_out  = F_in_CO2 * (1.0 - float(res['X_CO2'][-1]))
            F_tot_out  = F_CO2_out / max(y_CO2_out, 1e-30)
            F_H2O_out  = float(res['C_H2O'][-1]) * R_gas * T_out / P_Pa * F_tot_out
            F_H2O_prod = 2.0 * F_in_CO2 * float(res['X_CO2'][-1])
            F_H2O_ads  = u_s * rho_bed_ads * q_out
            bal_err    = (F_H2O_out + F_H2O_ads - F_H2O_prod) / max(F_H2O_prod, 1e-30) * 100
            print(f"    H2O balance [mmol/(m²·s)]:  produced={F_H2O_prod*1e3:.3f}  "
                  f"gas_out={F_H2O_out*1e3:.3f}  solid_out={F_H2O_ads*1e3:.3f}  "
                  f"err={bal_err:+.1f}%")
            q_init = np.interp(np.linspace(0, L_b, 150), res['z'], res['q'])
        else:
            print(f"  u_s={u_s*1e3:.3f} mm/s  FAILED  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")

        all_results_dp[label][i_us] = {'res': res, 'u_s': u_s}

print(f"\nAll done.  Total: {_fmt_seconds(time.perf_counter() - t_run_start)}")
# endregion


# region 5. PLOTS
# =============================================================================
SAVE_DIR = os.path.dirname(os.path.abspath(__file__))

def _savefig(filename):
    plt.savefig(os.path.join(SAVE_DIR, filename), dpi=150, bbox_inches='tight')

n_dp    = len(D_P_LIST)
pal_dp  = plt.cm.plasma(np.linspace(0.1, 0.85, n_dp))
markers = ['o', 's', '^', 'D', 'v', 'P']

X_eq  = equilibrium_conversion(T_K)
X_noSE = noSE['X_CO2_noSE'] * 100

# ── Plot A: CO2 conversion vs u_s ────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
for k, (label, results) in enumerate(all_results_dp.items()):
    us_ok, X_ok = [], []
    for i_us in range(len(U_S_LIST)):
        e = results.get(i_us)
        if e and e['res'] is not None:
            us_ok.append(e['u_s'] * 1e3)
            X_ok.append(float(e['res']['X_CO2'][-1]) * 100)
    if us_ok:
        ax.semilogx(us_ok, X_ok, marker=markers[k], color=pal_dp[k],
                    lw=2, ms=6, label=f'd_p = {label}')
ax.axhline(X_eq,   color='k',    lw=1,   ls=':',  alpha=0.6, label=f'Thermo. eq. ({X_eq:.1f}%)')
ax.axhline(X_noSE, color='grey', lw=1.5, ls='--', alpha=0.8, label=f'Fixed bed / u_s=0 ({X_noSE:.1f}%)')
ax.set_xlabel('u_s [mm/s]', fontsize=11)
ax.set_ylabel('CO2 conversion [%]', fontsize=11)
ax.set_title(f'CO2 conversion vs solid velocity  |  T_in = {T_C} °C  |  GHSV = {GHSV}  |  U_a = {U_a:.0f} W/(m³·K)',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('dp_plotA_conversion_vs_us.png');  plt.show()

# ── Plot B: Sorbent utilisation vs u_s ───────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
for k, (label, results) in enumerate(all_results_dp.items()):
    us_ok, util_ok = [], []
    for i_us in range(len(U_S_LIST)):
        e = results.get(i_us)
        if e and e['res'] is not None:
            res    = e['res']
            T_K_e  = T_K
            q_out  = float(res['q'][0])
            p_H2O_peak = float(np.max(res['C_H2O'])) * R_gas * T_K_e / 1e5
            qs_peak    = float(q_star(T_K_e, np.array([max(p_H2O_peak, 1e-8)]))[0])
            util = q_out / max(qs_peak, 1e-10) * 100
            us_ok.append(e['u_s'] * 1e3)
            util_ok.append(util)
    if us_ok:
        ax.semilogx(us_ok, util_ok, marker=markers[k], color=pal_dp[k],
                    lw=2, ms=6, label=f'd_p = {label}')
ax.axhline(100, color='grey', lw=1.5, ls='--', label='q = q* (fully saturated)')
ax.set_xlabel('u_s [mm/s]', fontsize=11)
ax.set_ylabel('Sorbent utilisation  q(z=0) / q*(p_H2O_max)  [%]', fontsize=11)
ax.set_title(f'Sorbent utilisation vs solid velocity  |  T_in = {T_C} °C  |  GHSV = {GHSV}',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('dp_plotB_sorbent_utilisation.png');  plt.show()

# ── Plot C: Peak temperature rise vs u_s ─────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
for k, (label, results) in enumerate(all_results_dp.items()):
    us_ok, dT_ok = [], []
    for i_us in range(len(U_S_LIST)):
        e = results.get(i_us)
        if e and e['res'] is not None:
            us_ok.append(e['u_s'] * 1e3)
            dT_ok.append(float(np.max(e['res']['T'])) - T_K)
    if us_ok:
        ax.semilogx(us_ok, dT_ok, marker=markers[k], color=pal_dp[k],
                    lw=2, ms=6, label=f'd_p = {label}')
dT_noSE = float(np.max(noSE['profile']['T'])) - T_K
ax.axhline(dT_noSE, color='grey', lw=1.5, ls='--', alpha=0.8,
           label=f'Fixed bed ({dT_noSE:.1f} K)')
ax.set_xlabel('u_s [mm/s]', fontsize=11)
ax.set_ylabel('ΔT_max = T_peak − T_in  [K]', fontsize=11)
ax.set_title(f'Peak temperature rise vs solid velocity  |  T_in = {T_C} °C  |  GHSV = {GHSV}',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('dp_plotC_peak_temperature.png');  plt.show()

# ── Plot D: q(z) and q*(z) profiles at u_s = 2 mm/s ─────────────────────────
# Shows how well each d_p tracks adsorption equilibrium along the bed.
i_us_plot = np.argmin(np.abs(U_S_LIST - 2e-3))   # closest to 2 mm/s
u_s_plot  = U_S_LIST[i_us_plot]

fig, (ax_q, ax_qs) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O loading profiles at u_s = {u_s_plot*1e3:.1f} mm/s  |  T_in = {T_C} °C',
             fontsize=11)
for k, (label, results) in enumerate(all_results_dp.items()):
    e = results.get(i_us_plot)
    if e is None or e['res'] is None:
        continue
    res    = e['res']
    z      = res['z']
    q_prof = res['q']
    p_H2O  = res['C_H2O'] * R_gas * res['T'] / 1e5
    qs_prof = q_star(res['T'], p_H2O)
    ax_q.plot(z, q_prof,  color=pal_dp[k], lw=2, label=f'd_p = {label}')
    ax_qs.plot(z, qs_prof, color=pal_dp[k], lw=2, label=f'd_p = {label}')

ax_q.set_xlabel('z [m]');  ax_q.set_ylabel('q [mol/kg]')
ax_q.set_title('Solid H2O loading  q(z)');  ax_q.legend(fontsize=8);  ax_q.grid(True, alpha=0.3)
ax_qs.set_xlabel('z [m]');  ax_qs.set_ylabel('q* [mol/kg]')
ax_qs.set_title('Equilibrium loading  q*(T, p_H2O) along bed')
ax_qs.legend(fontsize=8);  ax_qs.grid(True, alpha=0.3)
plt.tight_layout()
_savefig(f'dp_plotD_q_profiles_us{u_s_plot*1e3:.1f}mms.png');  plt.show()

# ── Plot E: Axial profiles per u_s — one line per d_p ────────────────────────
# For each selected u_s, shows how particle diameter changes the bed profiles.
US_PLOT_TARGETS = [1.0e-3, 2.0e-3, 4.0e-3, 8.0e-3]   # [m/s]

for u_s_tgt in US_PLOT_TARGETS:
    i_us_e = np.argmin(np.abs(U_S_LIST - u_s_tgt))
    u_s_e  = U_S_LIST[i_us_e]

    fig, axes = plt.subplots(2, 2, figsize=(13, 9))
    fig.suptitle(f'Axial profiles at u_s = {u_s_e*1e3:.1f} mm/s  |  '
                 f'T_in = {T_C} °C  |  GHSV = {GHSV}  |  U_a = {U_a:.0f} W/(m³·K)',
                 fontsize=11)
    ax_X, ax_q2, ax_T, ax_p = axes[0,0], axes[0,1], axes[1,0], axes[1,1]

    for k, (label, results) in enumerate(all_results_dp.items()):
        e = results.get(i_us_e)
        if e is None or e['res'] is None:
            continue
        res    = e['res']
        z      = res['z']
        p_H2O  = res['C_H2O'] * R_gas * res['T'] / 1e5   # [bar]
        qs_prof = q_star(res['T'], p_H2O)
        lbl = f'd_p = {label}'

        ax_X.plot(z, res['X_CO2'] * 100,     color=pal_dp[k], lw=2, label=lbl)
        ax_q2.plot(z, res['q'],               color=pal_dp[k], lw=2, label=lbl)
        ax_q2.plot(z, qs_prof, color=pal_dp[k], lw=1.5, ls='--', alpha=0.5)
        ax_T.plot(z, res['T'] - 273.15,       color=pal_dp[k], lw=2, label=lbl)
        ax_p.plot(z, p_H2O * 1e3,             color=pal_dp[k], lw=2, label=lbl)

    ax_X.set_xlabel('z [m]');  ax_X.set_ylabel('CO2 conversion [%]')
    ax_X.set_title('CO2 conversion along bed');  ax_X.legend(fontsize=8);  ax_X.grid(True, alpha=0.3)

    ax_q2.set_xlabel('z [m]');  ax_q2.set_ylabel('q [mol/kg]')
    ax_q2.set_title('Solid H2O loading  (solid) and q*  (dashed)');  ax_q2.legend(fontsize=8);  ax_q2.grid(True, alpha=0.3)

    ax_T.axhline(T_C, color='grey', lw=1.5, ls='--', alpha=0.7, label=f'T_wall = {T_C} °C')
    ax_T.set_xlabel('z [m]');  ax_T.set_ylabel('T [°C]')
    ax_T.set_title('Temperature profile');  ax_T.legend(fontsize=8);  ax_T.grid(True, alpha=0.3)

    ax_p.set_xlabel('z [m]');  ax_p.set_ylabel('p_H2O [mbar]')
    ax_p.set_title('Gas-phase H2O partial pressure');  ax_p.legend(fontsize=8);  ax_p.grid(True, alpha=0.3)

    plt.tight_layout()
    _savefig(f'dp_plotE_axial_us{u_s_e*1e3:.1f}mms.png');  plt.show()
# endregion
