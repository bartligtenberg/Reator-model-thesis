...
def solve_mpb(u_s, T_K, T_wall=None, max_iter=200, tol=1e-3, N=100, q_init=None, kldf_scale=1.0):
    ...
    for it in range(max_iter): # start gauss seidel loop
        q_fn = interp1d(z_grid, q_prof, kind='linear',
                        bounds_error=False, fill_value=(q_prof[0], q_prof[-1]))   # turn currect best-guess profiles for Q and T into array on Z-grid, with interpolation since the functions are continuous. the fill_value argument ensures that if the function is evaluated outside the bounds of z_grid, it will return the first or last value of q_prof, respectively. This prevents errors when the solver evaluates at points outside the grid.
        ...
        if gas_dominates:
            ...
            def gas_rhs(z, y):
                ...
                Kl  = float(K_LDF(T_l,  np.array([p_H2O]))[0]) * kldf_scale
                ads = Kl*(qs - q_l)
            ...
            def solid_rhs(zeta, q_arr):
                ...
                Kl  = float(K_LDF(T_local,  np.array([p_H2O_l]))[0]) * kldf_scale
            ...
            def gas_rhs_no_T(z, y):
                F_CO2_l = max(y[0], 0.0);  F_H2_l  = max(y[1], 0.0) # clip so they dont become zero by numerical overshoot
                F_CH4_l = max(y[2], 0.0);  F_H2O_l = max(y[3], 0.0)
                T_l     = max(float(T_fn(z)), 200.0)  # from gauss seidel iteration previous solid iteration
                q_l     = max(float(q_fn(z)), 0.0) # get solid loading from previous iteration
                ...
                r   = float(reaction_rate_SI(T_l, np.array([p_CO2]), np.array([p_H2]),
                                              np.array([p_CH4]), np.array([p_H2O]))[0])
                qs  = float(q_star(T_l, np.array([p_H2O]))[0])
                Kl  = float(K_LDF(T_l,  np.array([p_H2O]))[0]) * kldf_scale   # for this K_ldf experiment
                ads = Kl*(qs - q_l)

                return [
                    -rho_bed_cat * r,           #RHS: returns derivatives in mol/(m2 * s) for each species, which is later used for solver.
                    -4.0*rho_bed_cat * r,
                    +rho_bed_cat * r,
            ...
            def solid_rhs_with_T(zeta, y_arr):
                ...
                Kl  = float(K_LDF(T_val, np.array([p_H2O]))[0]) * kldf_scale
                ads = Kl*(qs - q_val)
    ...
    # Decompose F_H2O into reaction and ads/desorption contributions
    # dF_H2O/dz = 2·ρ_cat·r  −  ρ_ads·ads
    # positive ads = adsorption (removes H2O from gas), negative = desorption (adds H2O)
    ads_fine      = K_LDF(T_fine, p_H2Of) * kldf_scale * (q_star(T_fine, p_H2Of) - q_fine)
    F_H2O_rxn     = cumulative_trapezoid(2.0 * rho_bed_cat * r_fine, z_fine, initial=0.0)
    F_H2O_des     = cumulative_trapezoid(-rho_bed_ads * ads_fine,     z_fine, initial=0.0)
    p_H2O_rxn     = F_H2O_rxn / F_totf * P_bar   # >0: H2O added by reaction
    p_H2O_des     = F_H2O_des / F_totf * P_bar   # >0: net desorption, <0: net adsorption

    return dict(z=z_fine, C_CO2=C_CO2f, C_H2=C_H2f, C_CH4=C_CH4f,
                C_H2O=C_H2Of, q=q_fine, T=T_fine, r=r_fine, X_CO2=X_CO2,
                p_H2O_rxn=p_H2O_rxn, p_H2O_des=p_H2O_des,
    ...
    for i_us, u_s in enumerate(U_S_LIST):
        ...
        if res is not None:
            ...
            print(f"  u_s={u_s*1e3:.4f} mm/s  X={X_out:.1f}%  "
                  f"q(0)={q_out:.3f}  q_sat={sat:.2f}{flag}  T_max={T_max:.1f} C  "
...
# ── Second pass: K_LDF × 0.1 ─────────────────────────────────────────────────
print("\n--- Running with K_LDF × 0.1 ---")
all_results_10x = {}
for T_C in T_IN_LIST:
    T_K = T_C + 273.15;  T_wall = T_K
    q_init = _q_physics_init(T_K)['q'][::-1]
    for i_us, u_s in enumerate(U_S_LIST):
        res = solve_mpb(u_s, T_K, T_wall=T_wall, q_init=q_init, kldf_scale=0.1)
        if res is not None:
            tag   = "ok" if res['converged'] else "not-conv"
            regime = "gas" if res['gas_dominates'] else "solid"
            X_out = float(res['X_CO2'][-1]) * 100
...
# ── Plot 7: K_LDF profile along z for u_s = 3 mm/s ──────────────────────────
e_kldf = all_results.get((T_C_PROF, 0))
if e_kldf and e_kldf['res']:
    r_kldf   = e_kldf['res']
    T_kl     = r_kldf['T']
    p_H2O_kl = r_kldf['C_H2O'] * R_gas * T_kl / 1e5          # mol/m³ → bar (ideal gas)
    K_raw    = K_LDF_vec(T_kl, p_H2O_kl, W0_DA, E_DA, n_DA)  # uncapped
    K_capped = K_LDF(T_kl, p_H2O_kl)                          # capped at K_LDF_MAX

    fig, axes_k = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(f'K_LDF along reactor  |  u_s = 3 mm/s  |  T_in = {T_C_PROF} °C', fontsize=11)

    axes_k[0].plot(r_kldf['z'], K_capped, lw=2, color='steelblue', label='K_LDF (capped)')
    axes_k[0].plot(r_kldf['z'], K_raw,    lw=2, color='steelblue', ls='--', alpha=0.5, label='K_LDF (uncapped)')
    axes_k[0].axhline(_K_LDF_MAX, color='red', lw=1, ls=':', label=f'cap = {_K_LDF_MAX} s⁻¹')
    axes_k[0].set_xlabel('z [m]', fontsize=11);  axes_k[0].set_ylabel('K_LDF [1/s]', fontsize=11)
    axes_k[0].set_title('LDF coefficient');  axes_k[0].legend(fontsize=9);  axes_k[0].grid(True, alpha=0.3)

    axes_k[1].plot(r_kldf['z'], p_H2O_kl * 1e3, lw=2, color='darkorange', label='p_H2O')
    axes_k[1].set_xlabel('z [m]', fontsize=11);  axes_k[1].set_ylabel('p_H2O [mbar]', fontsize=11)
    axes_k[1].set_title('H2O partial pressure (drives K_LDF via dq*/dp)');  axes_k[1].legend(fontsize=9);  axes_k[1].grid(True, alpha=0.3)

    plt.tight_layout()
    _savefig(f'flux_plot7_K_LDF_T{T_C_PROF}C_us3mms.png');  plt.show()
...
for i_us in range(len(U_S_LIST)):
    u_s_val   = U_S_LIST[i_us]
    u_s_label = f'{u_s_val*1e3:.1f}'

    # ── Plot 8 ───────────────────────────────────────────────────────────────
    fig, axes8 = plt.subplots(2, 2, figsize=(13, 9))
    fig.suptitle(f'K_LDF sensitivity  |  u_s = {u_s_label} mm/s  |  T_in = {T_C_PROF} °C',
                 fontsize=11)
    for store, scale, lbl, col in _cases:
    ...
    for ax, (store, scale, lbl, col) in zip(axes9.flat, _cases):
        ...
        r = e['res']
        p_tot = r['C_H2O'] * R_gas * T_K_p8 / 1e2
        p_rxn = r['p_H2O_rxn'] * 1e3
        p_des = r['p_H2O_des'] * 1e3
        ax.plot(r['z'], p_tot, color=col,       lw=2,   label='p_H2O total')
        ax.plot(r['z'], p_rxn, color=col,       lw=1.5, ls='--', label='from reaction')
        ax.plot(r['z'], p_des, color='dimgray', lw=1.5, ls=':',  label='from ads/desorption')
        ax.axhline(0, color='black', lw=0.8, ls='-', alpha=0.4)
        ax.set_xlabel('z [m]', fontsize=10);  ax.set_ylabel('p_H2O [mbar]', fontsize=10)
        ax.set_title(lbl, fontsize=10);  ax.legend(fontsize=8);  ax.grid(True, alpha=0.3)
    plt.tight_layout()
    _savefig(f'flux_plot9_H2O_decomposition_us{u_s_label}mms.png');  plt.show()
# endregion
