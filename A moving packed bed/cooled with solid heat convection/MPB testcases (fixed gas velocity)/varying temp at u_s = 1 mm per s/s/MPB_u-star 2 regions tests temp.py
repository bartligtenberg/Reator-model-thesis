...
U_a    = 2000.0         # [W/(m³_bed·K)]  — volumetric wall heat transfer coefficient
                       # Rough estimate: h_wall ≈ 25 W/(m²·K) × (4/d_b) = 2000 W/(m³·K)
...
# --- Test: limiting case u_s → 0  (all values << u_s* ≈ 0.47 mm/s) ---
# u_s = 0 is the fixed-bed reference, computed separately by _compute_noSE
U_S_LIST  = np.array([1e-3])                 # [m/s] = 1 mm/s  (fixed)
T_IN_LIST = list(range(200, 341, 20))        # [°C]  200, 220, ..., 340
...
cmap  = plt.cm.viridis
pal_T = cmap(np.linspace(0.1, 0.85, len(T_IN_LIST)))
i_us  = 0   # only one u_s value

# ── Plot 1: Axial profiles for all temperatures ──────────────────────────────
...
for j, T_C in enumerate(T_IN_LIST):
    ...
    r   = e['res']
    lbl = f"T_in = {T_C} °C"
    axes[0,0].plot(r['z'], r['C_CO2']*1e3, color=pal_T[j], lw=2, label=lbl)
    axes[0,1].plot(r['z'], r['q'],          color=pal_T[j], lw=2, label=lbl)
    axes[1,0].plot(r['z'], r['X_CO2']*100,  color=pal_T[j], lw=2, label=lbl)
    axes[1,1].plot(r['z'], r['r']*1e3,      color=pal_T[j], lw=2, label=lbl)
...
plt.tight_layout()
_savefig('plot1_axial_profiles.png');  plt.show()

# ── Plot 2: CO2 conversion vs T_in ───────────────────────────────────────────
fig, ax = plt.subplots(figsize=(9, 5))
T_ok, X_ok, X_eq_ok, X_noSE_ok = [], [], [], []
for T_C in T_IN_LIST:
    e = all_results.get((T_C, i_us))
    if e and e['res']:
        m = get_metrics(e)
        if m:
            ...
            X_eq_ok.append(equilibrium_conversion(T_C+273.15))
            X_noSE_ok.append(noSE_results[T_C]['X_CO2_noSE']*100 if T_C in noSE_results else None)
ax.plot(T_ok, X_ok, marker='o', color='tab:blue', lw=2, ms=7,
        label=f'MPB (u_s = {U_S_LIST[0]*1e3:.1f} mm/s)')
ax.plot(T_ok, X_eq_ok, color='grey', lw=1.5, ls=':', label='Thermodynamic equilibrium')
noSE_pairs = [(T, X) for T, X in zip(T_ok, X_noSE_ok) if X is not None]
if noSE_pairs:
    ax.plot([t for t,_ in noSE_pairs], [x for _,x in noSE_pairs],
            marker='s', color='tab:orange', lw=2, ls='--', ms=6, label='Fixed bed (u_s = 0)')
ax.set_xlabel('T_in [°C]', fontsize=11);  ax.set_ylabel('CO2 conversion [%]', fontsize=11)
ax.set_title(f'MPB  |  u_s = {U_S_LIST[0]*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  — CO2 conversion vs T_in',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('plot2_conversion_vs_Tin.png');  plt.show()
...
for T_C in T_IN_LIST:
    ...
    if e and e['res']:
        m = get_metrics(e)
        if m:
            T_ok3.append(T_C);  util_ok.append(m['sorbent_util']*100)
ax.plot(T_ok3, util_ok, marker='o', color='tab:green', lw=2, ms=7,
        label=f'u_s = {U_S_LIST[0]*1e3:.1f} mm/s')
...
ax.set_title(f'MPB  |  u_s = {U_S_LIST[0]*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  — Sorbent utilisation',
             fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('plot3_sorbent_utilisation_vs_Tin.png');  plt.show()

# ── Plot 4: H2O profiles for all temperatures ────────────────────────────────
fig, (ax_q, ax_h) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(f'H2O profiles  |  u_s = {U_S_LIST[0]*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)', fontsize=11)
for j, T_C in enumerate(T_IN_LIST):
    e = all_results.get((T_C, i_us))
    if e is None or e['res'] is None:
        continue
    r   = e['res'];  T_K = e['T_K'];  lbl = f"T_in = {T_C} °C"
    ax_q.plot(r['z'], r['q'],                        color=pal_T[j], lw=2, label=lbl)
    ax_h.plot(r['z'], r['C_H2O']*R_gas*T_K/1e2,     color=pal_T[j], lw=2, label=lbl)
...
plt.tight_layout()
_savefig('plot4_H2O_profiles.png');  plt.show()

# ── Plot 5: Temperature profiles for all temperatures ────────────────────────
fig, ax_T = plt.subplots(figsize=(9, 5))
fig.suptitle(f'Temperature profiles  |  u_s = {U_S_LIST[0]*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)',
             fontsize=11)
for j, T_C in enumerate(T_IN_LIST):
    e = all_results.get((T_C, i_us))
    if e is None or e['res'] is None:
    ...
    lbl    = f"T_in = {T_C} °C ({regime})"
    ax_T.plot(r['z'], r['T'] - 273.15, color=pal_T[j], lw=2, label=lbl)
...
plt.tight_layout()
_savefig('plot5_temperature_profiles.png');  plt.show()

# ── Plot 6: Peak temperature rise vs T_in ────────────────────────────────────
fig, ax6 = plt.subplots(figsize=(9, 5))
T_ok6, dT_ok, dT_noSE_ok = [], [], []
for T_C in T_IN_LIST:
    e = all_results.get((T_C, i_us))
    if e and e['res']:
        m = get_metrics(e)
        if m:
            ...
            if T_C in noSE_results and noSE_results[T_C].get('profile') is not None:
                dT_noSE_ok.append(float(np.max(noSE_results[T_C]['profile']['T'])) - (T_C + 273.15))
            else:
                dT_noSE_ok.append(None)
ax6.plot(T_ok6, dT_ok, marker='o', color='tab:red', lw=2, ms=7,
         label=f'MPB (u_s = {U_S_LIST[0]*1e3:.1f} mm/s)')
noSE_dT_pairs = [(T, dT) for T, dT in zip(T_ok6, dT_noSE_ok) if dT is not None]
if noSE_dT_pairs:
    ax6.plot([t for t,_ in noSE_dT_pairs], [dT for _,dT in noSE_dT_pairs],
             marker='s', color='tab:orange', lw=2, ls='--', ms=6, label='Fixed bed (u_s = 0)')
ax6.set_xlabel('T_in [°C]', fontsize=11)
ax6.set_ylabel('ΔT_max = T_peak − T_in  [K]', fontsize=11)
ax6.set_title(f'MPB  |  u_s = {U_S_LIST[0]*1e3:.1f} mm/s  |  U_a = {U_a:.0f} W/(m³·K)  — peak temperature rise',
              fontsize=10)
ax6.legend(fontsize=9);  ax6.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('plot6_Tmax_vs_Tin.png');  plt.show()
# endregion
