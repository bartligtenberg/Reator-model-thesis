...
# --- MPB scan parameters ---
GHSV_LIST = np.concatenate([
    np.logspace(np.log10(0.1), np.log10(0.5), 5)[:-1],   # 0.10–0.34, coarse (4 pts)
    np.logspace(np.log10(0.5), np.log10(2.0), 20),        # 0.50–2.00, fine  (20 pts)
])  # [m³_STP/(kg_ads·h)]
u_s_fixed = 1.0e-3     # [m/s]  — fixed solid velocity (1 mm/s)
T_IN_LIST = [280]      # [°C]
...
print(f"  GHSV range: {GHSV_LIST[0]:.2f} – {GHSV_LIST[-1]:.2f} m³_STP/(kg_ads·h)")
# endregion
...
for T_C in T_IN_LIST:
    ...
    print(f"\n{'='*60}")
    print(f"  T_in = {T_C} C  |  u_s = {u_s_fixed*1e3:.1f} mm/s (fixed)  |  U_a = {U_a:.0f} W/(m3·K)")
    print(f"  GHSV sweep: {GHSV_LIST[0]:.2f} – {GHSV_LIST[-1]:.2f} m³_STP/(kg_ads·h)")
    print(f"{'='*60}")

    q_init = None
    for i_ghsv, GHSV in enumerate(GHSV_LIST):
        u_g_STP_i = GHSV * M_ads / 3600.0 / A_b
        u_g       = u_g_STP_i * (T_K / T_STP)
        ...
        noSE_results[(T_C, i_ghsv)] = _noSE

        t0  = time.perf_counter()
        res = solve_mpb(u_s_fixed, u_g, T_K, C_in_CO2, C_in_H2, C_in_CH4,
        ...
        if res is not None:
            ...
            print(f"  GHSV={GHSV:.3f}  u_g={u_g*1e3:.1f}mm/s  u_s*={u_s_star*1e3:.3f}mm/s  "
                  f"X={X_out:.1f}%  q(0)={q_out:.3f}  T_max={T_max:.1f}C  "
            ...
            print(f"  GHSV={GHSV:.3f}  FAILED  ({dt:.1f}s, ETA {_fmt_seconds(eta)})")
        all_results[(T_C, i_ghsv)] = {'res': res, 'u_s': u_s_fixed, 'GHSV': GHSV,
                                       'u_g': u_g, 'T_K': T_K, 'C_in_CO2': C_in_CO2,
                                       'T_wall': T_wall}

    # ── Downward sweep: retry failed GHSV points with reversed continuation ──
    n_failed = sum(1 for i in range(len(GHSV_LIST))
                   if all_results.get((T_C, i), {}).get('res') is None)
    if n_failed:
        print(f"\n  Downward sweep: retrying {n_failed} failed point(s)...")
        q_init_down = None
        for i_ghsv in range(len(GHSV_LIST) - 1, -1, -1):
            e = all_results.get((T_C, i_ghsv))
            if e is None:
                continue
            if e['res'] is not None:
                q_init_down = np.interp(np.linspace(0, L_b, 150),
                                        e['res']['z'], e['res']['q'])
            elif q_init_down is not None:
                GHSV_i     = e['GHSV']
                u_g_i      = e['u_g']
                u_s_star_i = u_g_i * (P_Pa / (R_gas * T_K)) * Cp_mix_est / (rho_bed_tot * Cp_cat)
                t0  = time.perf_counter()
                res = solve_mpb(u_s_fixed, u_g_i, T_K, C_in_CO2, C_in_H2, C_in_CH4,
                                T_wall=T_wall, q_init=q_init_down)
                dt  = time.perf_counter() - t0
                if res is not None:
                    X_out  = float(res['X_CO2'][-1]) * 100
                    q_out  = float(res['q'][0])
                    ...
                    regime = 'gas' if res['gas_dominates'] else 'solid'
                    tag    = 'ok' if res['converged'] else 'not-conv'
                    print(f"  GHSV={GHSV_i:.3f}  u_g={u_g_i*1e3:.1f}mm/s  "
                          f"u_s*={u_s_star_i*1e3:.3f}mm/s  "
                          f"X={X_out:.1f}%  q(0)={q_out:.3f}  T_max={T_max:.1f}C  "
                          f"[{regime}-dom, {tag}, {res['n_iter']}it, "
                          f"err={res['conv_err']:.2e}]  ({dt:.1f}s)  [↓]")
                    e['res'] = res
                    q_init_down = np.interp(np.linspace(0, L_b, 150),
                                            res['z'], res['q'])
                else:
                    print(f"  GHSV={GHSV_i:.3f}  FAILED (↓)  ({dt:.1f}s)")
...
n_ghsv     = len(GHSV_LIST)
i_ghsv_ref = n_ghsv // 2   # reference GHSV index for axial profile noSE overlay
# pick ~6 evenly spaced GHSV indices for axial profile overlays
i_plot     = np.linspace(0, n_ghsv - 1, min(12, n_ghsv), dtype=int)
pal        = plt.cm.plasma(np.linspace(0.1, 0.85, len(i_plot)))
pal2       = cmap(np.linspace(0.1, 0.85, len(T_IN_LIST)))

_p0_ref = noSE_results.get((T_C_PROF, i_ghsv_ref), {}).get('profile')
_ghsv_ref_label = f'GHSV≈{GHSV_LIST[i_ghsv_ref]:.2f}'

# ── Plot 1: Axial profiles ───────────────────────────────────────────────────
fig, axes = plt.subplots(2, 2, figsize=(13, 9))
fig.suptitle(f'MPB axial profiles  |  T_in={T_C_PROF}°C  |  u_s={u_s_fixed*1e3:.1f}mm/s  |  '
...
if _p0_ref is not None:
    axes[0,0].plot(_p0_ref['z'], _p0_ref['C_CO2']*1e3, color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
    axes[0,1].plot(_p0_ref['z'], _p0_ref['q'],          color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
    axes[1,0].plot(_p0_ref['z'], _p0_ref['X_CO2']*100,  color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
    axes[1,1].plot(_p0_ref['z'], _p0_ref['r']*1e3,      color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
...
for j, T_C in enumerate(T_IN_LIST):
    ...
    for i_g in range(n_ghsv):
        e = all_results.get((T_C, i_g))
        if e and e['res']:
            m = get_metrics(e)
            if m:
                ghsv_ok.append(e['GHSV']);  X_ok.append(m['X_CO2']*100)
        e_noSE = noSE_results.get((T_C, i_g))
        if e_noSE:
            X_noSE_ok.append(e_noSE['X_CO2_noSE']*100)
    if ghsv_ok:
        ax.semilogx(ghsv_ok, X_ok, marker=markers[j], color=pal2[j],
    ...
    if X_noSE_ok and len(X_noSE_ok) == len(ghsv_ok):
        ax.semilogx(ghsv_ok, X_noSE_ok, color=pal2[j],
                    lw=1.5, ls='--', alpha=0.8, label=f'{T_C}°C u_s=0 (fixed bed)')
ax.set_xlabel('GHSV  [m³_STP/(kg_ads·h)]', fontsize=11)
ax.set_ylabel('CO2 conversion [%]', fontsize=11)
ax.set_title(f'MPB  |  T_in={T_C_PROF}°C  |  u_s={u_s_fixed*1e3:.1f}mm/s  |  '
             f'U_a={U_a:.0f}W/(m³·K)  — CO2 conversion vs GHSV', fontsize=10)
ax.legend(fontsize=9);  ax.grid(True, alpha=0.3);  ax.set_ylim(0, 105)
plt.tight_layout()
_savefig('plot2_conversion_vs_GHSV.png');  plt.show()
...
for j, T_C in enumerate(T_IN_LIST):
    ...
    if ghsv_ok:
        ax.semilogx(ghsv_ok, util_ok, marker=markers[j], color=pal2[j],
...
ax.set_title(f'MPB  |  u_s={u_s_fixed*1e3:.1f}mm/s  |  U_a={U_a:.0f}W/(m³·K)  — Sorbent utilisation',
             fontsize=10)
...
if _p0_ref is not None:
    ax_q.plot(_p0_ref['z'], _p0_ref['q'],                        color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
    ax_h.plot(_p0_ref['z'], _p0_ref['C_H2O']*R_gas*T_K_prof/1e2, color='k', lw=2, ls='--', label=f'u_s=0 ({_ghsv_ref_label})')
...
for k, i_g in enumerate(i_plot):
    ...
    lbl    = f"GHSV={e['GHSV']:.2f} ({regime})"
    ax_T.plot(r['z'], r['T'] - 273.15, color=pal[k], lw=2, label=lbl)
if _p0_ref is not None:
    ax_T.plot(_p0_ref['z'], _p0_ref['T'] - 273.15, color='k', lw=2, ls='--',
              label=f'u_s=0 ({_ghsv_ref_label})')
ax_T.axhline(T_C_PROF, color='grey', lw=1.5, ls='--', alpha=0.8,
             label=f'T_in = T_wall = {T_C_PROF} °C')
...
# ── Plot 6: Peak temperature rise vs GHSV ────────────────────────────────────
fig, ax6 = plt.subplots(figsize=(9, 5))
for j, T_C in enumerate(T_IN_LIST):
    ghsv_ok, dT_ok, dT_noSE_ok = [], [], []
    for i_g in range(n_ghsv):
        e = all_results.get((T_C, i_g))
        if e and e['res']:
            m = get_metrics(e)
        ...
        e_noSE = noSE_results.get((T_C, i_g))
        if e_noSE and e_noSE.get('profile') is not None:
            dT_noSE_ok.append(float(np.max(e_noSE['profile']['T'])) - (T_C + 273.15))
    if ghsv_ok:
        ax6.semilogx(ghsv_ok, dT_ok, marker=markers[j], color=pal2[j],
                     lw=2, ms=6, label=f'{T_C}°C (MPB)')
    if dT_noSE_ok and len(dT_noSE_ok) == len(ghsv_ok):
        ax6.semilogx(ghsv_ok, dT_noSE_ok, color=pal2[j],
                     lw=1.5, ls='--', alpha=0.8, label=f'{T_C}°C u_s=0 (fixed bed)')
ax6.set_xlabel('GHSV  [m³_STP/(kg_ads·h)]', fontsize=11)
ax6.set_ylabel('ΔT_max = T_peak − T_in  [K]', fontsize=11)
ax6.set_title(f'MPB  |  u_s={u_s_fixed*1e3:.1f}mm/s  |  U_a={U_a:.0f}W/(m³·K)  — peak temperature rise',
              fontsize=10)
ax6.legend(fontsize=9);  ax6.grid(True, alpha=0.3)
plt.tight_layout()
_savefig('plot6_Tmax_vs_GHSV.png');  plt.show()
# endregion
