...
def reaction_rate_SI(T, p_CO2, p_H2, p_CH4, p_H2O):
    ...
    DEN     = (1.0
               ...
               + K_H2  * p_H2_s**0.5
               + K_mix * p_CO2_s)
    return k * (p_CO2_s * p_H2_s)**0.5 * f_eq / DEN**2 * 1000.0 * 0.8
...
h_Xeq, = ax2.plot([0, 120], [X_eq, X_eq],
                   color='m', lw=1.5, ls='--',
...
ax1.set_title(
    f'Sorption-enhanced CO₂ methanation — 5%Ni2.5%Ce13X\n'
    f'Validation of Wei Fig. 5.10 — T = {T_C} °C, P = {P_bar} bar, '
    f'GHSV = {GHSV:.0f} mL/(g·h)\n'
    f'Feed: 10 mL/min H₂ · 2.5 mL/min CO₂ · 81.5 mL/min CH₄ · 6 mL/min N₂  '
    f'(CH₄ = 94 mol% ≡ full CO₂ conversion)',
    fontsize=9
)
