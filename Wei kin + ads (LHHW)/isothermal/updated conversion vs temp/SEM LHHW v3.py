...
active_fraction = 0.1      # fraction of m_cat_total treated as catalytically/sorptively active [-]
...
for T_C in T_LIST:
    ...
    print(f"  Bed:   L = {L_b*100:.0f} cm,  d = {d_b*100:.1f} cm,  "
          f"m_cat = {m_cat*1000:.1f} g  (active_fraction = {active_fraction:.0%} of {m_cat_total*1000:.1f} g)")
...
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), 'Figure_1.png'), dpi=300)
plt.show()
# endregion
