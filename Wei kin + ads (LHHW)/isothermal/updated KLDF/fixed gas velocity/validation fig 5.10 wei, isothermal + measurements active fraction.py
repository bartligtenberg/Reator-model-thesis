...
active_fraction = 0.16              # fraction of m_cat_total that is catalytically active (Ni) [-]
...
rho_ads = 768                    # liquid water density        [kg/m³]
...
def rhs_sem(t, y, T, u, C_in_CO2, C_in_H2, C_in_CH4, C_in_H2O):
    ...
    # 1D transient PFR balance: eps_b * dC/dt = -u*dC/dz + rho * stoich * r
    # Reaction scales with rho_cat (active_fraction of bed); adsorption scales with the full rho_bed (sorbent = all material).
    dCdt_CO2 = (-u * (C_CO2 - C_CO2_up) / dz + rho_cat * (-1) * r) / eps_b
    dCdt_H2  = (-u * (C_H2  - C_H2_up)  / dz + rho_cat * (-4) * r) / eps_b
    dCdt_CH4 = (-u * (C_CH4 - C_CH4_up) / dz + rho_cat * (+1) * r) / eps_b
    dCdt_H2O = (-u * (C_H2O - C_H2O_up) / dz + rho_cat * (+2) * r
...
plt.tight_layout()
fname = f'validation_fig5_10_wei_isothermal_with_measurements_T{T_C:.0f}C_af{active_fraction:.0%}.png'
